...
Be suspicious whenever you see an
...
## And notice this
(mm2 <- which(A < 5, arr.ind = TRUE))
A[mm2]

## cf with
which(A < 5, arr.ind = FALSE) ## this is the default
@
...
types. In fact, there need not be any rectangular-like structure. Many functions in R  return lists.
...
This shows a few common elements (and some math and LaTeX\footnote{I am using package \texttt{latex2exp}; you might also want to check the xdivr package: \Burl{https://cran.r-project.org/web/packages/xdvir/index.html}}). Again, this is just a very tiny set of what is available:
