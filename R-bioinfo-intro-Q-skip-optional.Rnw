...
%% From R-bioinfo-intro-Q.Rnw, but I experiment with leaving only the most important, and moving the rest as subsection, as we are going too slow.
\synctex=1
...
\newcommand{\optactivities}{{\textcolor {Green} {Optional activities}}}

\newcommand{\skipinclass}[1]{{\textcolor {Periwinkle} {Skip in class #1}}}
...
%% Have paragraphs show as subsubsubsections
\makeatletter
...
\author{Ramon Diaz-Uriarte\thanks{Dept.\ of Biochemistry, Universidad
    Aut\'onoma de Madrid, Spain, \Burl{https://ligarto.org/rdiaz},
    \texttt{r.diaz@uam.es}}}
...
This work is Copyright, \copyright, 2014 to present, Ramon Diaz-Uriarte,
...
This document is to be used as a crash and relatively quick introduction to R, with a clear Bioinformatics bias\footnote{OK, there is an example with
...
\item We then (sections \ref{readingr} to \ref{plotsplots}) cover in some
  ...
  that they'd rather see this first, so this comes first now.
...
Some parts are marked as ``\activities''. Please make sure to do them. \textbf{Some exam questions will come from the \activities.}


Some sections/subsections/subsubsections/whatever that we will not cover in class (nor would be in an exam) are indicated in  \textcolor{Green}{green, and should have a descriptive text that clearly labels them as optional}.  Often associated to the \textcolor{Green}{optional sections} are some activities marked as ``\optactivities''. We rarely have time to cover or discuss them; you are encouraged to try them, but unless explicitly stated, we won't cover this in class (nor, if there is an exam, will they be part of an exam).


\subsubsection{Changes starting on 2025-10-16: moving material to \skipinclass{whatever}}
\label{sec:chang-start-2025}

As we are (course 2025-2026) going too slow, I will concentrate the boring syntax part only on the \textbf{absolutely, nonnegotiable, essential stuff}, so that we have time to discuss more interesting and larger programming examples. Thus, I will move material that we will not cover in class to their own subsections (or subsubsubsections, or whatever), indicated as ``\skipinclass{Whatever (subsub)section title}''.


\textbf{You probably do not want to skip this while studying}; I am skipping it in class (unless you have questions), for the sake of efficiency, but this is not optional material; it is just material that introduces some redundancy and can help you understand/remember the material better.
You can expect that some exam questions will come from the \skipinclass{material}. (Again: the only optional material (the material that will not be in the exam), is indicated as \optactivities\  or has section (subsection, or whatever) in \textcolor{Green}{green and has text that indicates that it is optional}.)
...
\Burl{https://github.com/rdiaz02/R-bioinfo-intro/blob/master/R-bioinfo-intro-Q.Rnw}. (Note that in the
github repo you will not see the PDF %% HTML,
or R-bioinfo-intro-Q.R files,
...
an \textbf{excellent intro}. This document is available with your installation of R, and you want to take a look at it.
There are many freely available documents (of variable
...
\item[Programming] As it says, just focus on programming R:
  \begin{itemize}
  \item ``fasteR: Fast Lane to Learning R!'': an R tutorial for beginners by Matloff: \Burl{https://github.com/matloff/fasteR}. If you are in a hurry, start here. (Note, though, that I think that some of the books Matloff recommends are not the best choices.)
  ...
  \item \textit{Advanced R, 2nd ed}. Wickham. (If you go to the web page for the
    book, in github, you can download the complete sources and build your
    own pdf).   \Burl{https://adv-r.hadley.nz/}
  \item \textit{Mastering software development with R}. Peng, Cross, Anderson. \Burl{https://leanpub.com/msdr}. The title is very descriptive.

    \item \textit{R for Data Science, 2nd ed}. Wickham, \c{C}etinkaya-Rundel, Grolemund. \Burl{https://r4ds.hadley.nz/}. Emphasis on data transformations and plots. Heavily uses the tidyverse.
...
\item[Stats and some programming] Introductory statistics (or introductory
  ...
  \item \textit{R in Action, 3rd ed}. Kabacoff. \Burl{https://www.manning.com/books/r-in-action-third-edition}.

  \end{itemize}
\item[Linear models et al.] Linear models are fundamental in
  statistics. And fascinating.
  \begin{itemize}
  \item \textit{An R companion to applied regression, 3rd edition}. Cox and
...
There is also a list of books I think are not worth it or do not given you anything the ones above already give you, in a much better way; ask me
about them in class.
...
There are a variety of ways of interacting and using R. For its ease of use, I will introduce RStudio. We
want to use the "Dektop", that you can download from here:
\Burl{http://www.rstudio.com/products/rstudio/download/}.

Note, though, I do not especially recommend RStudio. In fact, I do not use RStudio at all: see section \ref{guis}.


\subsection{Editors and  ``GUIs'' for \R, et al.}\label{guis}

\subsubsection{Editors for R}\label{editors}
Ah, this is a nice topic for a long, passionate, conversation. In this course, by
default, I'll introduce RStudio. I will, however, use Emacs + ESS
(\Burl{http://ess.r-project.org/}) except for a few minutes (those when I
introduce RStudio). Some Mac users are very happy just the default, plain,
interface provided by R under Mac OS X.  If you love vim, there is Vim-R-plugin
(\Burl{http://www.vim.org/scripts/script.php?script_id=2628};
\Burl{https://hpcc.ucr.edu/manuals/hpc_cluster/terminalide/}). If you use VSCode
(or, probably better, VSCodium \Burl{https://vscodium.com/}), there are several extensions  for working with
R: \Burl{https://marketplace.visualstudio.com/items?itemName=Ikuyadeu.r}.
For those used to Eclipse, there is a plug-in designed to
work with R: StatET
(\Burl{https://projects.eclipse.org/projects/science.statet}).   And some Windows users like Tinn-R (\Burl{https://github.com/jcfaria/Tinn-R} or \Burl{https://tinn-r.org/en/}, as the github repo does not show any commits for the last four years?);
I used to use Tinn-R in R courses I taught 12 to 15 years ago, but I think it has
lost ground to RStudio.   And then, if you
look around, there are other options for other editors.


% Another popular
% interface is JGR (\Burl{http://www.rforge.net/JGR/}).
% RKward
% (\Burl{http://rkward.sourceforge.net/wiki/Main_Page}) is also popular in some
% places (this was originally Linux-only, but not anymore).
% ; %% (an outdated list is available from
...
If you plan to spend a fair amount of time doing Bioinformatics, then you'll
spend a fair amount of time programming, probably using a variety of languages
(R, Python, C, JS, etc). Becoming used to a programmer-friendly editor that
``understands'' all of the languages you use is thus worth it, and these skills
transfer to any of your other text-editing needs (emails, writing papers, writing
the yearly letter to the three wise men, etc). This is why some people like to
use a single editor for most/all editing tasks. But then, some other people jump
around (they use RStudio for R, Eclipse for Java, and maybe Spider for
Python). Emacs is an editor and then a lot of other things (that is what I use,
for programming, writing, calendar, email, etc); if you use Emacs then Emacs +
ESS is the perfect combination (if you use Vim or Eclipse or VSCodium, see links
above).  % Kate is another great editor that understands many languages and it easy
% to submit code to an R process running in the terminal, but it lacks some nice
% features that RStudio and Emacs+ESS have.  You get to choose.


Note, though, that one thing is syntax highlighting (and syntax highlighting for
R is available for many, many editors) and another is the ability to interact
with an R session, provide shortcuts for displaying help, offering object
browsers, etc. Of course, you are the one who must weight the choices.


The summary: I definitely prefer Emacs (+ ESS), but in this course I will not
attempt to teach you Emacs + ESS. So if you do not know Emacs,\ldots well, go
learn it! Or try RStudio\footnote{Oh, but I must quote Rodney Sparapani here:
  ``My students are unconvinced and use RStudio but I am not allowed to fail them
  for their merely youthful indiscretions ;o)'' in
  \Burl{https://stat.ethz.ch/pipermail/ess-help/2021-September/012970.html}}. However,
if you like VSCodium, use it. If you prefer Eclipse, then use Eclipse. Etc. But I
might not be able to help you.

\paragraph{Suggestions if you want to try Emacs:}

\begin{enumerate}
\item Install Emacs, a recent version (try for it to be at least version 28 or above). Most Linux distributions have a recent enough Emacs. For Windows and macOS a good option might be Vincent Goulet's binaries, which also include additional packages such as ESS: \url{https://gitlab.com/emacs-modified}.
\item Do the Emacs tutorial: start Emacs, and type \texttt{C-h t} (that is: press the Control key and, while holding it, press ``h''; release and press ``t''). This will give you a basic understanding of Emacs keybindings and common terminology. \textbf{Do not skip this ---if you want to use Emacs, that is; skip if you use lesser editors}.
\item All you have now is bare bones Emacs. What about starter kits? In  other words, ``Emacs bundles of packages + config with all batteries  included''? I do not generally recommend them. Some people (mainly ex-Vim  users) enjoy Emacs Doom or Spacemacs, but they might later get in the  way.   If I had to recommend one, I'd try ``Emacs Bedrock'':  \url{https://sr.ht/~ashton314/emacs-bedrock/}; it uses some settings and  packages I do not use, but seems very minimal, and it looks like it would  easily allow modifying the configuration ``as the user grows in knowledge  and power''. An even more minimal approach is Protesilaos Stavrou's  ``Emacs: a basic and capable configuration'':  \url{https://protesilaos.com/codelog/2024-11-28-basic-emacs-configuration/}.


\item Take a loot at the documentation for ESS, ``Emacs Speaks Statistics'', the way we interact with R: \url{https://ess.r-project.org/Manual/ess.html}.
\item To learn more about Emacs itself:
  \begin{itemize}

 \item The short intro course ``Getting started with Emacs'', by Marie-Hélène Burle, available from \url{https://mint.westdri.ca/emacs/top_intro}.

 \item The book ``Use GNU Emacs. The Plain Text Computing Environment'', by Keith Waclena: \url{https://www2.lib.uchicago.edu/keith/emacs/}. Freely available as HTML, PDF, Epub ---yeah, there are unfinished chapters, but it is very complete as it is now.

    \item The book ``Mastering Emacs'', by Mickey Petersen, that you can purchase from \url{https://www.masteringemacs.org/}. It is not free, but if you are going to use Emacs, it is probably well worth it.


  \item The Emacs manual. It is long, but there are many things you can skip. It is a very well written and organized manual, with lots of cross-references, that allows you to choose what to read. I think it is the best manual I've ever read. Available as PDF, HTML, and Info.

\item I'd start with the short intro course to get a quick overview and  then dive into one of Petersen's or Waclena's, or both. And I'd probably intersperse jumping to the Emacs manual to read specific chapters or sections of things that particularly interest you (e.g., read about Dired in Petersen or Waclena, then go to the Emacs Manual). Repeat as needed.
  \end{itemize}
\item Continue the journey on your own.
  \end{enumerate}

\subsubsection{GUIs for analyses}
\label{sec:guis-analyses}


Note that all of the above have a different purpose from tools such as R
Commander (\Burl{http://socserv.mcmaster.ca/jfox/Misc/Rcmdr/}) which is a basic
statistics GUI for R. In this course we will rarely (if at all) use R Commander,
since these notes are focused on programming and using R from the command
line. However, you might want to play around with R
...
To complete this section, a great and recent comparison of available R GUIs is
available from \Burl{http://r4stats.com/articles/software-reviews/} and an
overview of all the reviews is available here
\Burl{http://r4stats.com/articles/software-reviews/r-gui-comparison/}. (Some of
the links in some of the reviews, say to RKWard might not work; google
for them).

What would I use if I had to use a GUI for R? R Commander, followed by
RKWard. Why? These are the reasons:

\begin{description}
\item[R Commander] Produces ``standard'' R code, which helps to make the
  transition to using R without a GUI. It offers exceptional support
  for building linear models (not surprising, given J.\ Fox is the main author of
  R Commander). Uses the R you have installed (so it requires installing R and a
  bunch of packages).
\item[RKWard] Using Kate can offer something approaching Emacs+ESS or RStudio.
  Produces ``standard'' R code of the analyses (but it seems it can also save the
  dialog box entries). But compared to R Commander, it offers fewer analyses via
  menus in the GUI.

\item[Rattle] Main focus is data mining, and does not offer as much support for
  classical and intro stats.
\item[JASP] Does not produce (at least as of now) R code. Cannot run R code
  directly. Installs its own R version and set of packages. It makes it easy to
  use Bayesian methods (maybe too easy?). Here, though, is an enthusiastic comment
  about
  JASP. \Burl{https://paulspector.com/i-like-teaching-statistics-with-jasp/}.
\item[jamovi] Does not produce ``standard'' R code, but uses its own, jamovi
  style syntax (as in package \texttt{jmv}). Sharing just the R code for data
  manipulations, though, seems not easy. JASP and jamovi share many things, and a
  common history. See discussion here
  \Burl{https://www.researchgate.net/post/Has_anyone_used_JASP_and_Jamovi} and
  \Burl{https://forum.jamovi.org/viewtopic.php?f=2&t=669}. % Between
  % the two, I'd probably choose jamovi simply because it can give you R code (and
  % Mac OS users would probably also find it easier because it does not require to
  % install X11). This is an interview with one of its lead developers: \Burl{http://blog.efpsa.org/2017/03/23/introducing-jamovi-free-and-open-statistical-software-combining-ease-of-use-with-the-power-of-r/}.
\item[BlueSky] Does not produce ``standard'' R code (rather, it heavily relies on
  the ``tidyverse'').
\item[Deducer] Very complex installation (and depends on Java). Its development
  has stalled and it is no longer actively maintained, and there are some things
  that no longer work, specially with newer Java versions (see thread in
  \Burl{https://groups.google.com/g/deducer/c/va5YnHNEw_0}).

\end{description}


And a few details about the criteria above, and why they matter (at least to me):
\begin{description}
\item[Capability of producing R output] I value that a tool produces R code so
  that students, or myself, can, if they want, modify the code directly. This can
  help when learning R. And this makes it possible to share analyses with
  colleagues who use R (regardless of the GUI). And this is a crucial element of
  reproducible research.
\item[Having a way to interact with R (run R code directly)] Related to the
  former, if we want to modify an analysis, even if we are using a GUI, it might
  be simpler to just change a minor piece of the code. For this, you need the
  system to provide you a way to interact with the R process directly.
\item["Standard" R] This is in contrast to producing ``tidyverse'' code or code
  that is specific to a GUI (as jamovi's jmv). There are many reasons why I think
  the tidyverse is not a good idea for teaching (see
  \Burl{https://github.com/matloff/TidyverseSkeptic}); I also tend to use the
  tidyverse sparingly outside of a teaching context. See below.
\item[Not reinstalling R] I already have one or more versions of R
  installed. Installing software that installs yet another version of R with a
  large set of packages is not ideal.
\end{description}
...
Finally, and although this is not a complete editor, for quick and dirty console
use of R you might want to use radian (\Burl{https://github.com/randy3k/radian});
this is what I sometimes use in my scratchpads.

\subsubsection{tidyverse}\label{sec:tidyverse}

This was mentioned above, and some students ask. So I'll anticipate.

We have
referred to \Burl{https://github.com/matloff/TidyverseSkeptic} (and the longer
version:
\Burl{https://github.com/matloff/TidyverseSkeptic/blob/master/READMEFull.md}).
Another short summary of the problems, which I very much share, is in
\Burl{https://stackoverflow.com/a/61393520}: basically, lack of flexibility,
unnecessary complexity even for simple operations, and lack stability.  You can
read a longer debate in \Burl{https://news.ycombinator.com/item?id=20362626},
where you'll see arguments against and in favor of. I'll cherry-pick on that
thread to mention some of my other arguments. They are mostly along the lines, of
for example, the comments in \Burl{https://news.ycombinator.com/item?id=20364056}
(interestingly, I also was introduced to R, after having used S-Plus for two
years, by one of R core developers in a class he taught) and those below by the
same person and others (e.g.,
\Burl{https://news.ycombinator.com/item?id=20369518} and
\Burl{https://news.ycombinator.com/item?id=20363863}): I find the tidyverse
limiting and complicating and it hides details in base R --details that are not
that complicated, and details that you will appreciate understanding for, say,
manipulating R's statistical functions output or for reading the R that forms the core of R itself-- plus it created other problems
with nonstandard evaluation.


With regards to the famous pipe, I first liked it
when I saw it (I use it in the shell a lot) but then found it hard to debug programs
that use it, and to comment/uncomment code, and I have an opinion similar to the
one in \Burl{https://news.ycombinator.com/item?id=20369115} and earlier and later
comments by the same person (this could be a personal bias: I tend to
conceptualize code around nested operations and function composition, and don't
dislike lisp's parentheses). And yes, I prefer \texttt{data.table} (for both
speed, but also for syntax and semantics), though we won't be covering it in this
course. By the way, tidyverse is not necessarily faster than base R (and sometimes it is slower); and, as just mentioned, if I care about speed I switch to \texttt{data.table}.

And let me return to an additional important problem: the (too frequent for my taste) deprecation of features (which is mentioned also in a link above: \Burl{https://stackoverflow.com/a/61393520} ).  I have code in base R that runs perfectly well 10 or 15 years later; I've had tidyverse code show deprecation notices 6 months after it was written.

Of course, there are other very different opinions:
\Burl{http://varianceexplained.org/r/teach-tidyverse/}, \Burl{https://cdmuir.netlify.app/post/2019-07-09-tidyverse/},
\Burl{https://towardsdatascience.com/rethinking-how-we-teach-the-tidyverse-da09a3a23934}.


Summary: little tidyverse here. I strongly believe that a solid foundation in base R \textbf{must} come first.
...
and see how instructions are clearly given there. The involve doing
something like
<<eval=FALSE>>=
...
% See
% \Burl{https://cran.r-project.org/web/packages/BiocManager/index.html}.
% But
...
\begin{enumerate}
\item Start R (just R, not RStudio) from your operating system.
\item Make sure you are able to do this operation: 3 + 2
\item Now, start RStudio
\item In RStudio, locate the console (bottom left, maybe?). Add 3 and 2
  again.
\item Now, open a new R Scritp (File, Open, New File, R Script). Write the
  following
...
\end{verbatim}
  ...
  and execute it. Go to ``Code'' and use ``Run selected line(s)'' and
  ``Run region''

\item In the console, type \texttt{help(rnorm)} (and then enter). What happens?
\item Can you see how you are offered suggestions as you type?
\item Install a package. Do it using \texttt{install.packages}. You can
  also try using RStudio. The package to install is called ``varSelRF''.

\item Install the \texttt{limma} package from Bioconductor.

\item Open this file (this file is ``R-bioinfo-intro-Q.Rnw'') in Rstudio. What
  kind of file is this? Can you run code from it? Could you generate the pdf?
...
\subsection{\activities}
\label{sec:activities-1}

\begin{enumerate}
\item In class, we will give a broad overview of:
  \begin{enumerate}
  \item vectors
  \item matrices
  \item arrays
  ...
  \textbf{Before} we do so, though, try to think what each one of those terms
  might refer to. And keep those terms in mind as you go over the next activities.

\item Type (do not copy-and-paste!) the example above in a new script and then
  execute it. If you get syntax errors, make sure you verify what you
  wrote. If (but only if) you become desperate, copy-paste the code into a
  script and execute it (or into a file and run it).


\item Play with the following and try to understand what is happening. We
...
\item And what about that \texttt{apply}?

  From your experience with other programming languages, this might make
  sense:

<<activ1_b, eval=TRUE, echo=TRUE, results=FALSE>>=

## For the sake of the example, use only the first 10 rows
## of randomdata
rd2 <- randomdata[1:10, ]
...
\item We haven't said it explicitly, but when we use \texttt{apply} we are
  passing a function (our anonymous function or \texttt{myfunction} as above) to
  another function. This is something very common and easy in R: passing
  functions to other functions.


\item Try this on your own. What is happening here?
<<difffx12>>=

f1x <- function(v, theclass) t.test(v ~ theclass)$p.value

apply(rd2, 1, function(z) f1x(z, theclass = class))

apply(rd2, 1, f1x, theclass = class)

## And this?
f2x <- function(clase, w) t.test(w ~ clase)$p.value

apply(rd2, 1, f2x, clase = class)
...
v1 <- 1 + 2
@
\noindent You can also use \code{=} for assignment, but I strongly prefer not
to. \activities: why might it not be a good idea to use \code{=} for assignment?
...
\label{sec:activities-2}

\begin{enumerate}
\item Make sure you type every single one of the expressions above and
  that all of them make sense to you. Do not skip any one. If things do
  not make sense, ask in class.
\end{enumerate}
...
Now let's use the help to   learn about the help system (and yes, read or
...
apply(v3, 1, mean) ## this is an interesting one
log("23")
rnorm("a")
lug(23)
rnorm(23, 1, 1, 1, 34)
...
\subsubsection{\activities}
\label{sec:activities-5a}

\begin{enumerate}
\item Make sure you try to understand the error messages from the above
  expressions. Some will be complicated, and we will explain them. Others
  should be very easy to understand.

  Type each one of the statements on its own. Or copy the above in a
  script, and execute line by line.
...
yourself in the future). Please, make sure your code is legible and respects
...
offer ways of tidying your code, and there are tools that can help you here
(\Burl{https://lintr.r-lib.org/}); you might want to have your editor alert you to coding style problems with the help of lintr (RStudio might have it integrated already?). You can also take a look at \Burl{http://yihui.name/formatR}.


\activities: show in class how lintr works (in my case with Emacs+ESS). Show, for instance:
\begin{itemize}
\item using ``='' for assignment;
\item not using spaces around (infix) operators
\item putting spaces after parenthesis
\item unused and unbound variables
\item cyclomatic complexity
\item 80-column rule
  \end{itemize}

  \subsubsection{Commenting code}\label{sec:code-comments}
And what about comments? My preferences: signature and purpose right before the function, comments inside the function. See details here: \Burl{https://docs.racket-lang.org/htdf/index.html\#\%28part._interface-definition\%29}: signature (input and output) and purpose.

(For an awesome course, that uses Racket, by Gregor Kiczales, see: \Burl{https://www.edx.org/course/how-to-code-simple-data})
%% Gregor Kiczales
% is even a package that can help
% you do it
...
only use \code{read.table}. What about \code{read.csv}? See the help.
...
\subsubsection{\activities}
\label{sec:activities-3}

\begin{enumerate}

\item Use \texttt{read.table} with \texttt{AnotherDataSet}. Make sure R is
  running in the same directory where the file is. And do it in two different
  ways:
  \begin{enumerate}
  \item Using \texttt{header = TRUE}
  \item Using \texttt{header = FALSE}
  \end{enumerate}

\item Repeat the above but starting R from a different directory or changing the
  working directory of R. (You can use \texttt{setwd} or use the pull down
  menus of RStudio: Session -> Set Working Directory). You will then use
  \texttt{read.table} passing the path of the file.
...
\item Substitute a value by nothing; in other words, just delete a value
  ...
  \code{read.table}.) Notice things will differ between character columns
  and numeric columns.
\end{itemize}
...
other special character string to identify them. \textbf{You will most
  likely want to use NA (or use some other character combination and be
  explicit), specially for character variables.} I also \textbf{specify the column separator} (argument \texttt{sep}) in most cases.


\subsubsection{\activities}
\label{sec:activities-4}

\begin{enumerate}
\item Copy the file ``AnotherDataSet'' and create a bunch of different
  files (say \texttt{f1.txt}, \texttt{f2.txt}, $\ldots$). You can actually
  do this using your operating system facilities or, else, and much
  simpler for this exercise, by opening \texttt{AnotherDataSet} in
  RStudio, then doing the changes explained below, and saving it with the
  name you want.
\item You want to use the following types of changes in a numeric column,
  like Age:
  \begin{enumerate}
  \item Replace one or more values by ``NA''
  \item Replace one or more values by a `` '' (a white space)
  \end{enumerate}
\item In each case above, make sure you can read the data set and check if
  things make sense.

  You might need to specify the separator between columns in some cases
  above. Notice that if you do not specify it, two or more spaces, for example, will be interpreted as a single column separator. From the help of \texttt{read.table}: ``If 'sep = ""' (the
          default for 'read.table') the separator is 'white space',
          that is one or more spaces, tabs, newlines or carriage
          returns.''


\item In ``AnotherDataSet.txt'' substitute the second entry for Sex by
  ``NA'' (without the quotes), save it (say ``ana1.txt'') and read the
  file (as, say, ``ana1''). Now, do a \texttt{summary} of that file. Is it obvious there is an
  ``NA'' (nope, it is not).  However, the NAs are properly taken into
  account as NAs. For example, if you do: \verb@ table(ana1$Sex,useNA="ifany") @.%
  If you ask for the object itself, you will see``<NA>''.
  And you can also check it by doing: \verb@which(is.na(ana1$Sex))@.%

\item Repeat the above, but substitute not by NA without quotes but by NA
  with quotes. Is that value of Sex considered an NA? %% Yes

\item Now, replace several consecutive values by nothing, for example, in
  the same row, replace by nothing in the same row Age and Sex (i.e., make
  sure you have at least two ``nothings'' one after the other.)

\item If things are confusing, look at this from the help of
  \texttt{read.table}: `` Blank fields are also considered to be
  missing values in logical, integer, numeric and complex
  fields.'' And notice also the following from the help:

\item Now, in the same row, for Age and Sex, replace the values by a space
  (not a ``nothing'', a space), while keeping columns separated by a
  tab.  And, when reading, use the option \texttt{na.strings = `` ''}
  (i.e., tell R that the character for NA is a space).
...
\Burl{http://cran.r-project.org/doc/manuals/R-data.html}).

For rectangular files, the first functions I'd try are \code{fread}, from package \code{data.table} and \code{read\_delim} from package \code{readr}.


For even larger things, of specialized uses, there are packages such as \code{ff} or \code{bigmemory}.




\subsection{Saving tables, data, and results}\label{savetables}
...
\subsubsection{\activities}
\label{sec:activities-5}

\begin{enumerate}
\item Make sure that you type and replicate what section \ref{saveRData} and
\ref{savetables} explain.
\item Use \texttt{save} to save:
  \begin{enumerate}
  \item A single object.
  \item Two or three objects
  \end{enumerate}
  Then, open another R session (you can do this from RStudio or use
  whatever editor or operating system facilities you use) and read
  them.

\item Read the documentation for \texttt{readRDS} (and \texttt{saveRDS}). Then:
  \begin{enumerate}
  \item Save one object using \texttt{saveRDS}
  \item Open another R session and read
    that object using \texttt{readRDS}
  \end{enumerate}
\item When could \texttt{saveRDS/readRDS} be more useful than \texttt{save/load}? % assign names of objects at will
  What is a problem of using \texttt{load}? %overwriting
  What is an advantage of using \texttt{save}? % saving names, multiple objects
\item Bring usage questions of \texttt{save.image}, \texttt{save},
  \texttt{read/saveRDS} to class.
\end{enumerate}
...
\item[Calling R from the shell] Open a shell, a command window, or however
  ...
  \verb@ R --vanilla -f script1.R > script1.Rout @

  I tend to use either one of these two, and then add things like ``nohup''
  before invoking \R, move it to the background, and also redirect standard error
  to the same file used for standard output (i.e., I type \code{\&> script1.Rout}
  instead of \code{ > script1.Rout}). In Windows, you might need to use
  \verb@  Rscript.exe @ or \verb@ R.exe @.

  Beware, the above are examples of simple invocations. There are many other
  options (including passing command line arguments; see \texttt{commandArgs}).
  This article contains details and examples: \Burl{https://swcarpentry.github.io/r-novice-inflammation/05-cmdline.html}.
...
\subsection{\activities}
\label{sec:activities-6}

\begin{enumerate}
\item Create a script that will do this:
  \begin{enumerate}
  \item Create a variable, called ``x'' that will take the value 4.
  \item Create another variable, called ``y'', that will take the value of
    \texttt{x * 2}
  \item Compute the mean of ``y'' (yes, yes, I know it is 8)
  \end{enumerate}
\item From RStudio, use \texttt{source} to execute the complete script.
\item Open R from your operating system, and repeat the last step.
\item Do an internet search for how to run R scripts non-interactively
  for your operating system, say from a shell/command window.

  I'll show it in Linux. If you use Windows, take a look at these
  links, for example:
  % \Burl{http://datacornering.com/how-to-run-r-scripts-from-the-windows-command-line-cmd/},


  \Burl{https://stackoverflow.com/a/3415135}
  \Burl{https://support.rstudio.com/hc/en-us/articles/218012917-How-to-run-R-scripts-from-the-command-line}.


  And these are two canonical entries from the R documentation (which are mentioned in one of the references above) and apply in general to all OSs:

  \Burl{https://cran.r-project.org/doc/manuals/R-intro.html\#Scripting-with-R}

  \Burl{https://cran.r-project.org/doc/manuals/R-intro.html\#Invoking-R-from-the-command-line}


  % How did I find them? The first two hits in a Google search for ``r cmd
  % batch windows''. \texttt{I am not necessarily recommending the above
  %   articles!!!}.


  You might want to try using the examples in those, or other links, to
  run scripts from the command window.

  Note that you can simplify them if you have R in the path. (Google for
  this: how to add a program to the path in your operating system. In
  Linux things ought to work out-of-the-box).
...
\subsection{\activities \ for this part}
\label{sec:activities-this-part}
In this part of the course, we will not add an ``Activities'' subsection
after each successive subsection. I expect you to type every single one of
the expressions below, make sure you get them to work, understand what
they are doing, and ask questions when things do not make sense.


Yes, every expression. You want to practice:
\begin{itemize}
\item Getting the syntax right.
\item Seeing and decoding error messages and rewriting things correctly.
\end{itemize}
...
\subsection{Creating vectors from other vectors and the recycling rule}
\label{sec:creat-vect-from}

<<recycling1>>=
v1 <- 11:16
v2 <- 1:6
## one on top of each other, so we can see what v1 + v2 does
rbind(v1, v2)
...
\activities: Use \texttt{rbind} in each one of the examples above.


\subsubsection{\skipinclass{:\ Slower going examples}}
...
## A vector with TRUE/FALSE
v1 < 3
## ... that we can explicitly assign
v2 <- (v1 < 3)

v11 <- c(1, 1, 3, 5)
## Equality, element-wise
v1 == v11
## ... or non-equality, element-wise
v1 != v11

## Very different question: are the the objects identical?
identical(v1, v11)

## Logical operations: negation, AND, OR, XOR
...
\subsubsection{\skipinclass{:\ Additional examples}}
<<>>=
v1 == v11
v1 != v11
!(v1 == v11)
...
\subsubsection{Logical values as 0, 1\label{log01} and \texttt{which}}
...
\paragraph{\skipinclass{:\ Additional examples}}
...
\subsubsection{Short-circuit logical operations}
\label{sec:shortc-logic-oper}

\code{\&\&} and \code{||} are short-circuit.


<<>>=

## FALSE short-circuits the AND
FALSE && log("a")

## TRUE short-circuits the OR
TRUE || log("a")
...
\activities : Try to run the above with a \code{\&} instead of \code{\&\&}.


\paragraph{\skipinclass{:\ Additional short-circuit examples and details}}


% Note, though, that ``Using vectors of more than one element in '\&\&' or '||' will give a warning (as from R 4.2.0), or an error if the environment variable '\_R\_CHECK\_LENGTH\_1\_LOGIC2\_' is set to a true value (this is intended to become the default in future).''
From R-4.4.1 this is now an error: ``Using vectors of more than one element in '\&\&' or '||' will give an error.'' (From the help).
...
\subsubsection{Do not use \code{T} for \code{TRUE} or \code{F} for
  \texttt{FALSE}}\label{no_t_for_true}
\activities \  Can you think why?

This is a short section: Never, ever, use \code{T} for \code{TRUE} or \code{F} for \texttt{FALSE}. You can count on at least one exam question about this.

%% https://stat.ethz.ch/pipermail/r-help/2008-November/180085.html


<<ltftf>>=
## Lots of other code before here

## This is not a bad idea per se.
T <- 0
F <- -1

## Using T and F as TRUE and FALSE are the bad ideas

## In explicit if
if (T) cat("This should be printed\n")
if (F) cat("This should NOT be printed\n")

## As arguments to functions
(x1 <- c(1, 2, NA))

mean(x1, na.rm = TRUE)
## This can be a VERY HARD to debugg bug
mean(x1, na.rm = T)

## In contrast, TRUE and FALSE cannot be overwritten
TRUE <- 0
FALSE <- 1

## For safety, remove them
rm(T, F)
@
...
distinct). We will soon see why this is very helpful. For now, see this:
...
\item By specifying the positions you want (or do not want): giving
  ...
  \item By using  logical vector (which is really very similar to the first).
...
## Of course, these two ways of removing are equivalent
w[-c(1, 3)]
w[-c(3, 1)]
@
...
## What are these doing? I would avoid these things
## (by having code fail if the length of the logical vector
## differs from the vector being indexed)
ages[c(FALSE, TRUE)]
...
Moral: Be suspicious whenever you see an
...
\subsubsection{\skipinclass{:\ Additional examples and info of float equality et al.}}
...
This might be an even simpler example:
...
These are two links that provide additional examples, and explanations at
different depths (from simple and intuitive to more detailed):
\Burl{https://floating-point-gui.de/} and \Burl{https://0.30000000000000004.com/}
...
## "unfactor" a factor of some numbers

(nr <- factor(c(4, 2, 8, 4, 1)))
as.numeric(as.character(nr))

@


\subsubsection{\skipinclass{:\ Additional examples and info}}
...
<<unfact2>>=
## Though there is a slightly better way?
## From ?factor
...
\subsubsection{\skipinclass{:\ Factors and symbols, colors, etc, in plots}}
...
\subsection{Matrices}
Vectors were one-dimensional. Matrices are two-dimensional, and arrays
...
(In the code below, the vector we create the matrix from is  the vector we create on the fly with \code{1:10})
<<>>=
matrix(1:10, ncol = 2)
## More explicit, better in code
matrix(1:10, nrow = 5, ncol = 2)
matrix(1:10, ncol = 2, byrow = TRUE)
@


\subsubsection{Combining vectors to create a matrix: \code{cbind, rbind}}
...
\paragraph{\skipinclass{:\ \texttt{rbind, cbind} also work  with matrices (if they are of the appropriate dimensions, of course)}}
...
\activities:\ Try to use \texttt{rbind} and \texttt{cbind} in matrices that are not of the appropriate dimensions. What happens?
...
columns. If you specify nothing for that dimension, it is returned completely. We've seen this already.


You can use a matrix to index another matrix.

<<>>=
A <- matrix(11:25, nrow = 5)
...
## And notice this
(mm2 <- which(A < 15, arr.ind = TRUE))
A[mm2]

## cf with
which(A < 15, arr.ind = FALSE) ## this is the default
@

\paragraph{\skipinclass{:\ Additional examples of accessing matrix by row/column indices and names}}
<<>>=
A <- matrix(11:25, nrow = 5)
A[1, ] ## first row
A[, 2] ## second column
...
\activities: What are \texttt{nrow} and \texttt{ncol} doing? Look at their help.
...
\activities: Use \texttt{apply} instead of \texttt{rowMeans, rowSums, colMeans, colSums} and verify they give the same solution for some matrices. Make sure you try this and understand what is happening.

<<rowmeapply>>=
A1 <- matrix(1:15, nrow = 5)

colSums(A1) == apply(A1, 2, sum)

## eh?
identical(colSums(A1), apply(A1, 2, sum))

class(colSums(A1))
class(apply(A1, 2, sum))

## We mentioned all.equal before
all.equal(colSums(A1), apply(A1, 2, sum))

@



\subsubsection{Matrices: dropping dimensions (or not)}
\label{sec:matr-dropp-dimens}

Carefully look at this. What is happening?
...
What is going on? When could this matter? What is the default behavior?




\subsubsection{\activities: More practice with matrices indexing matrices}

Play with the following and make sure you understand it

<<matinmat>>=
## Examples of indexing a matrix with a matrix

A <- matrix(1:15, ncol = 3) + 3
...
A[c(3, 1, 5, 5), c(3, 2, 1, 2)]

A[m2]

@

\subsubsection{\optactivities: Matrices indexing matrices, II (more advanced, possibly counterintuitive)}
<<matmatnoint>>=
## The above should be cristal clear. Now for something not
## intuitive (for me) ......

## What if we index a matrix with a matrix with
## more than 2 columns?
...
## Humm... it seems R uses the entries in m22 as just the
## position in the matrix (as if it were a vector).
## Let's check

## Two ways to turn a matrix into a vector
avv <- A
dim(avv) <- NULL
av <- as.vector(A)
...
## In fact, no need to turn matrix into vector
A[as.vector(m22)]
...
types. No need for any rectangular-like structure. Many functions in R  return lists.

<<tidy=FALSE>>=

(listB <- list(one.vector = 1:10,
               hello = "Hola",
               one.matrix = matrix(111:125, ncol = 5),
               another.list =
               list(a = 5,
                    b = factor(c("male",
                      "female", "female")))))
...
Removing elements from a list? Assign null to it:
<<listnull>>=
listB$one.vector <- NULL
names(listB)
@



\subsubsection{\skipinclass{:\ Additional list examples}}

<<>>=
listA <- list(a = 1:5, b = letters[1:3])
listA[1]
listA["a"] ## different from the ones below

listA[[1]]
listA[["a"]]
listA$a
@
\noindent so those are the basic ways of getting access to list
...
listB[[c(3, 4)]]
@
though I tend to find the last one confusing, even if it is well documented in the help (\texttt{?"[["})\footnote{We are not alone if we are surprised by this: \Burl{https://kbroman.org/blog/2013/04/02/xc53/}.}

I'd rather do
<<>>=
listB[[3]][1, 2]
@
so first access the matrix, and then the element that occupies first row and second column.

\vspace*{15pt}

To make sure we see this more clearly, do:
%""
<<eval=TRUE,echo=TRUE,results='hide'>>=
listB[[c(3, 11)]]
listB[[3]][11]
...
listB[[3]][ , 4, drop = FALSE]
listB[[3]][3, , drop = FALSE]
...
My recommendation, at least initially: do not try to be too smart when dealing with lists. Access each type of element, and then subset that element as needed.
...
Data frames are very common and handy for data analysis.


We can access elements of data frames as if they were matrices and as
if they were lists (where the list elements are the columns) (we could \skipinclass{ the next code block}, but this is so absolutely essential, that we will go over it once again ---but there is nothing new in the next code block):
...
Creating a data frame is easy:
<<>>=
(AC <- data.frame(ID = "a9", Age = 14, Sex = "M", Y = 17))

## And rbind and cbind also work with data frames
rbind(AB, AC)
AB2 <- rbind(AB, AB)
@
...
\activities : Try adding a new variable to a matrix. How would you do it?
...
\activities:\ Carefully look at the output from \texttt{data.matrix} and \texttt{as.matrix}: what is each one doing? When could each be useful?


% \subsubsection{\skipinclass\ \texttt{rbind, cbind} and data frames}
...
% \subsection{\skipinclass:\ Odds and ends}
...
% to make life much easier for this task. This is expanded in section \ref{sec:resh-data:-moving}
...
The basic plot function is \code{plot}. It's \textbf{help} can be slightly misleading and many additional arguments are explained in \code{par}.

\activities: Look at the help for \texttt{plot} and \texttt{par}:
<<helpplotpar, eval=FALSE>>=
?plot
?par
@


A good analogy to begin with is that of a canvas where you go adding
...
x <- 1:10
y <- sample(seq(from = 2, to = 20, length.out = 10))
...
This shows a few common elements (and some math and \LaTeX\footnote{I am using package \texttt{latex2exp}; you might also want to check the xdivr package: \Burl{https://cran.r-project.org/web/packages/xdvir/index.html}}). Again, this is just a very tiny set of what is available (and using \LaTeX will NOT be part of the exam):

<<plotssimple,fig.width=4, fig.height=4>>=
library(latex2exp) ## For "TeX", to use LaTeX
plot(x, y,
     main = TeX(r"(The title with some LaTeX: \frac{\bar{x}}{\gamma} = 3)"),
     ylab = "Label for the y axis",
     xlab = expression(paste("The x lab with some math: ", alpha = sqrt(beta))),
     pch = c(19, 1:9),
     cex = seq(from = 0.6, to = 3, length.out = 10),
     col = rainbow(10))
text(7, 9, "Some text", srt = 35)
abline(a = 3, b = 2.5, lty = 2, col = "blue")
abline(a = 13, b = -1.5, lty = 4, col = "salmon", lwd = 4)
...
Look at the help for \texttt{abline} and redo the above plot adding a blue
vertical line at x = 4; then add a green diagonal line of slope 1,
intercept 2. Now, add an orange vertical line at x = 30; what would you
need to show it?



\subsection{\skipinclass:\ Plots: Can we change colors, line types, point types, etc?}
...
text(11, 2.2, "col (from rainbow)", cex = 1.3)
@
...
\subsubsection{\activities}
\label{sec:activities-7}

Type all of the code above and make sure you can execute it without
problems (by sending to the interpreter the lines of code you want) and
that you really understand what is happening in each line. You might want
to look at the help for \texttt{text} and \texttt{points}.

Plot a few points, with coordinates (1, 3), (2, 7), (8, 9) and use
\texttt{type = ``b''}.
...
<<>>=
pdf(file = "one-plot.pdf", height = 4, width = 3)
plot(1:15, main = "a plot", col = rainbow(15))
dev.off()
@


\subsubsection{\activities}
\label{sec:activities-8}

\begin{enumerate}
\item Create a variable, x1, that takes 37 values between 2 and 4.
\item Create a variable, y1 that is equal to 3 times x1 + a random noise term that
  is normally distributed with mean 0 and standard deviation of 0.4.
\item Plot y1 as a function of x1. Use points and a line to connect
  consecutive points.
\item Save that plot as a png; call it ``my-pl.png''. Height and width
  should be 340.
\item Save that plot as a pdf; call it ``my-pl.pdf''. height and width
  should be 3 and 6 inches, respectively.

\item Now create the following three variables, z1, z2, z3. They all
  contain random deviates from a normal distribution of mean 3 and
  standard deviation 2; z1 has 10 values, z2 50 values, z3 1000 values.
\item Plot a histogram of each.
\item Produce png and pdf files with the plots. Yes, the pdf is a single
  pdf, with multiple pages. What about the png?
\end{enumerate}

\subsection{\skipinclass:\ Plots, plots, plots. Many types of plots}
...
find nicer. If you are interested, google for ``ggplot2''. There are
several books about it, including the next two ``ggplot2: elegant graphics
for data analysis'', by Wickham, its creator, and ``R graphics cookbook'',
by Chang, who has been heavily involved in ggplot2. The second one has a
web page with many recipes:
\Burl{http://www.cookbook-r.com/Graphs/}. Another popular package is
\textbf{lattice}, also with its own book. And there are comparison pages
of lattice and ggplot2. Another issue we haven't touched is choice of
colors, which is not a trivial thing (and there are a variety of color
...
An example where we use ggplot is provided in section \nameref{sec:metab-rate-long}.
...
\subsection{Example of reading data and plotting: Plotting the results from BLAST}\label{blast}


This is a quick example of something that is not really just a mere toy
example.  We will use output from alignment statistics from BLAST.  Let's take a quick look at a couple of things:
\begin{itemize}
...
The quick code. Please, make sure to look at the original data. I know (from the metadata) what columns 5 and 13 are.
...
plot(hit[, 13] ~ hit[, 3], xlab = "Percent. identity",
     ylab = "Bit score", main = "Bit score vs. % identity")
@
...
\subsubsection{\skipinclass{:\ A 3D plot and more details}}


A 3D plot\footnote{This is not a great, or
  even a decent, model of the relationship, which we can see, for
  instance, from the pattern of the deviations of the points from the
  plane. But the key for now is the ease of plotting.}
<<>>=
## I comment it out, so I can generate the notes without it stopping
if (FALSE) {
    library(car)
    scatter3d(hit[, 13] ~ hit[, 3] + hit[, 5], xlab = "Ident",
              zlab = "Length", ylab = "Score")
...
\subsubsection{\activities}
\label{sec:activities-9}


\begin{enumerate}
\item Make sure you type all of the commands above. That includes playing with
\texttt{scatter3d} (and you might need to install some packages; do it).

\item The figures we showed side by side: place them not side by side, but
  one above the other (i.e., in a configuration of one-column and
  two-rows).

  \item Change the histogram: make sure it goes to 1200 on the x axis, and
    the bars are of salmon color (the color is called ``salmon''). Add a
    big (say, size 9) blue solid point at coordinates x = 300, y = 100.
\end{enumerate}
...
get a quick idea of the spread of the points, etc. And I want to do it only for the birds. Oh, and I want to add the best fitting least-squares line.
...
<<plotjustbirds,fig.width=4, fig.height=4>>=
...
## Fit a line by OLS, show intercept and slope
summary(lm1 <- lm(logMetab ~ logBodyMass, data = birds))

## Note I reuse the expression!
plot(logMetab ~ logBodyMass, data = birds)
abline(lm1)

## Though we could have done (using package car)
...
## But the procedure above is a general one that will work
## even if John Fox had not written scatterplot.
...
\paragraph{\activities} Redo the previous plot but remove the
smooth lines (the smoothed line and associated conditional variance). Now
repeat, and make sure you only show a boxplot for metabolic rate (not body
mass).

\vspace*{20pt}


\subsubsection{\skipinclass{:\ More examples}}

You could do something very similar to what we did with the \texttt{car} function \texttt{scatterplot} starting with the basic plot function, but it
...
how the different colors by Class is done in the call to plot and how we set the
legends (\activities : why do we know the legend labels are correct?):
...
\paragraph{\activities} Could you add the regression line of
Metabolic rate on body mass? Suppose you wanted to add three regression
lines, one for birds (in salmon color), one for reptiles (green) and one
for all together, regardless of taxonomic group (thick black). Think how
you would do it. No need to do it if you don't want to but you might want
to play with it.
...
different argument for the call, and you get automatic legend, etc. Pay attention to the syntax, the ``|'':
...
% \paragraph{\skipinclass{: Transforming variables}}
...
% \paragraph{\skipinclass{: Only the birds! Selecting specific cases}}
...
% \paragraph{\skipinclass{: The regression only for the birds}}
...
\paragraph{\skipinclass{: How I read and saved the data?}\label{readanage}}
...
\skipinclass{:\ We will skip all of this. I will only briefly explain random number generators and using their seed.}
...
\paragraph{Random numbers for parallelized runs?} Yes, R provides those. See the
help for \texttt{mclapply} and \texttt{mcparallel}. These links contain more
info:
\Burl{https://bookdown.org/rdpeng/rprogdatascience/parallel-computation.html},
\Burl{https://cran.r-project.org/web/packages/dqrng/vignettes/parallel.html}.
...
\subsubsection{Using ggplot to display the box plot with dots}

To use ggplot, we first need to\footnote{We do not really need to, but not doing so would force us to do unnecessary gimnastics.} create a data frame

<<df_for_ggpl_x2hj4>>=

## Note I use somewhat different variable names
dftt <- data.frame(gene_A = GeneA,
                   gene_B = GeneB,
                   type   = Type)
@
...
<<ggplot_x2hj4, fig.width=6, fig.height=4>>=
library(ggplot2)

p1 <- ggplot(dftt,
             aes(x = type, y = gene_A, color = type)) +
    geom_boxplot() +
    geom_jitter(shape = 16, width = 0.1, height = 0)

## ?geom_jitter: yes, make sure to understand in what direction
## jittering is added.
p1


## Add the mean
p1 + stat_summary(fun.y = mean, geom = "point", shape = 23, size = 4)
## Did you see the "deprecated"
...
## And something probably silly and the yellow colour is a terrible
## choice
ggplot(dftt,
       aes(x = type, y = gene_A, color = type)) +
    geom_boxplot(notch = TRUE) +
    geom_jitter(shape = 16, width = 0.1, height = 0,
                col = c("yellow", "green")[dftt$type])
@


See more examples at \Burl{http://www.sthda.com/english/wiki/ggplot2-box-plot-quick-start-guide-r-software-and-data-visualization}

\subsubsection{\activities}
\label{sec:activities-10}

Repeat the t-test examples but the data now will come from a uniform
distribution. Is this problem well specified? %% what uniform
%% distribution? Also, when Gene B ... what do we mean?
Where do you need to change things in the code above? Do change it.


Use stripchart but make sure you use jittering so that points do not
overlap.

Get a better feeling for what jitter is doing by using the variables
\texttt{xo} and \texttt{class} below, and plotting them with and without
jittering (make sure you use a vertical plot, so that you have ``class''
in the x axis)

<<>>=
xo <- c(rep(1, 5), rep(2, 3), 3)
xo <- c(xo, 2 * xo, xo + rnorm(length(xo)))
class <- factor(rep(c("A", "B", "C"), rep(length(xo)/3, 3)))
@

<<eval=TRUE,echo=FALSE,results=FALSE, fig.width=7, fig.height=4>=
stripchart(xo ~ class, method = "jitter", vertical = TRUE)
@
...
%% FIXME: in many/most tables, change the 1, 2, 3, by letters or
%% something. Can be confusing if the levels are counts
\section{Tables}\label{tables}

\subsection{Tables: basics}\label{table-basics}

Tabulating data is a very common operation. Let's use again the data frame
we played with above (and remember to type the code; this is one implicit
\activities )
...
\subsection{More than two dimensions and \texttt{ftable}}

<<ftable0>>=
(x <-  data.frame(a = c("A1","A2","A2","A1","A2","A2","A1"),
                  b = c("B1","B2","B2","B1","B1","B2","B1"),
                  c = c("C1","C1","C2","C1","C2","C2","C1")))
## Equivalent
table(x)
xtabs(~ a + b + c, data = x)
@

But with more than two variables, sometimes we want ``flat tables'':
<<ftable1>>=
ftable(xtabs(~ a + b + c, data = x))
## same as ftable(table(x))
@

Eh, wait, I want variable ``c'' in the left-most border! Change order of variables
<<ftable2>>=
ftable(xtabs(~ c + a + b, data = x))
@


\subsection{Tables and missing values}
\label{sec:tabl-miss-valu0}
(This is examined in more detail below; what follows is basically a warning)

<<tabna0>>=
X <- c("X1", "X1", "X2", "X2", "X2", NA)
Y <- c("X1", "X1", "X2", "X2", "X2", 3)
which(is.na(X))
table(X)
table(X, useNA = "ifany")
table(Y, useNA = "ifany")
...
## with xtabs
xtabs(data = data.frame(X), addNA = FALSE)
xtabs(data = data.frame(X), addNA = TRUE)


(x4 <-  data.frame(U = c("A","B","B","A","B","B",NA),
                   W = c("d","e","e","d","d","e","d")))
table(x4)
table(x4, useNA = "ifany")
table(x4, useNA = "always")
xtabs(data = x4, addNA = FALSE)
xtabs(data = x4, addNA = TRUE)
@


\subsection{\textcolor {Green} {(Optional) Tables II}}
\label{tablesII}
Unless we have a lot of time:
\begin{itemize}
\item We will skip this in class
\item This will not be in the exam
\end{itemize}


\subsubsection{\textcolor {Green}  {(Optional) Recovering a table from a data frame}}
<<>>=
...
\subsubsection{\textcolor {Green} {(Optional) Recovering the data that produced the table}}
And what if we want to recover the data (in our case above, the dataframe \texttt{x}) and we have a data frame that already contains the frequencies? The following code is something you should be able to understand:

<<>>=
dfx[rep(1:nrow(dfx), dfx$Freq), c("a", "b", "c")]
@

The order of the data are not the same as in \texttt{x} and the rownames look strange, but the data are the same.


%% FIXME: what if we do not use all variables for indexing?
%% Dealt with in the activities of aggregate + xtabs
\subsection{\textcolor {Green} {(Optional) xtabs and ftable: summarizing arbitrary stuff}}\label{xtabs-arbitrary}
...
(Specially the examples with NAs are tricky.)


Suppose we have a data frame with values of some arbitrary variable for (some of) the
levels of some categorical variable. And we want a table (pay attention:
what is \texttt{expand.grid} doing?):

<<xtabs_ftable>>=
set.seed(1)
dfx3 <- expand.grid(F1 = c("A", "B"), Age = c(10, 20, 30),
                    Loc = c("k1", "m3"))
...
<<xtabs_ftable_na1>>=
dfx4 <- dfx3[-c(3, 12), ]
ftable(xtabs(Vx ~ ., data = dfx4))
@
Notice how the second row, first column (F1==A, Age == 20, Loc == k1) and the last row, second column, are now shown as 0.


And what if they are missing values (in Vx, not in the categorical variables)?

<<xtabs_ftable_na2>>=
dfx5 <- dfx3
dfx5[c(3, 12), "Vx"] <- NA
ftable(xtabs(Vx ~ ., data = dfx5))
@

% I now (2024-10) don't understand what I was trying to show
% since addNA changes nothing. Has the behavior of xtabs changed?
% Yes: the dfx5 behavior has changed from R-4.3 to R-4.4.1
% https://bugs.r-project.org/show_bug.cgi?id=17770#c11
% Think this carefully: is that always the behavior you want?

% Change that:

% <<xtabs_ftable_na3>>=
% ftable(xtabs(Vx ~ ., data = dfx5, addNA = TRUE))
% @

% And here? (Look for F1==A, Age == 20, Loc == k1)
% <<xtabs_ftable_na4>>=
% ftable(xtabs(Vx ~ ., data = dfx4, addNA = TRUE))
% @


So: one thing is having no observations (cero counts, if these were
contingency tables) and another is having missing values for those
combinations.  We will see some of this again in section
\ref{sec:activ-aggr-xtabs}.


I would argue that the table with \texttt{dfx4} is probably not
doing what we want: the 0 have different meanings, as there was actually
an entry, a set of value that actually had 0, whereas some other 0 are
coming from combinations that are not present. As we will see in section
\ref{sec:activ-aggr-xtabs}, this is probably something that ought to have
been considered when we created \texttt{dfx4}.


(Note that this is tricky stuff and has prompted some discussion, and changes as recent as from 2024-03: \Burl{https://bugs.r-project.org/show_bug.cgi?id=17770\#c11}. So recent, that previous versions of these notes had additional examples that now have identical behavior and are no longer shown.)

% (There is another option for \texttt{xtabs}: \texttt{na.action}; look at
% the examples for \texttt{xtabs})




\optactivities \ And what happens if you do

<<xtabs_arbitr_5>>=
ftable(xtabs(Vx ~ Age + Loc, data = dfx3))
@


And you could then produce a nice LaTeX table using \texttt{toLatex} after
loading (and installing, if needed) package \texttt{memisc}:

<<xtabs_ftable_lat, eval=FALSE>>=
toLatex(ftable(xtabs(Vx ~ ., data = dfx3)))
@



\subsection{\textcolor {Green} {(Optional) Tables and missing values, more examples and details}}
\label{tablena}
...
Let us go back to tables and missing values
...
## with xtabs
xtabs(data = data.frame(x1), addNA = FALSE)
xtabs(data = data.frame(x1), addNA = TRUE)
@

Now in two dimensions

<<tabna2>>=
(x3 <-  data.frame(a = c(1,2,2,1,2,2,1),
                  b = c(1,2,2,1,1,2,1)))
table(x3)

(x4 <-  data.frame(a = c(1,2,2,1,2,2,NA),
                  b = c(1,2,2,1,1,2,1)))
table(x4)
table(x4, useNA = "ifany")
table(x4, useNA = "always")
...
table(x3, useNA = "always")
xtabs(data = x3, addNA = TRUE)
@

% Note, by the way, that the behavior of \texttt{xtabs(data = x3, addNA =
%   TRUE)} is similar to what we saw with \texttt{xtabs(Vx ~ ., data = dfx4,
%   addNA = TRUE)}: we get 0 in the cells with no combinations.



(Finally: when there are NAs, some extra care might need to be taken,
specially if the NAs are present both in the response and the grouping
variables. See this recent thread, for instance, with a possible bug
report:
\Burl{https://stackoverflow.com/questions/61240049/inconsistency-of-na-action-between-xtabs-and-aggregate-in-r} and
\Burl{https://bugs.r-project.org/bugzilla/show_bug.cgi?id=17770}).


\subsection{\optactivities}
\label{sec:activities-11}
Make sure you type all of the examples above. Also, if LaTeX is anything
that interests you, do the last example using \texttt{toLatex}.


%% FIXME: from a xtab to a data.frame of individual cases
%% https://stackoverflow.com/a/36499608

\subsection{\textcolor {Green}{(Optional) Tables, III}}\label{tablesIII}
Unless we have a lot of time:
\begin{itemize}
\item We will skip this in class.
\item This will not be in the exam.
...
% skip this is if you do not care for now. We will skip this in class.
...
%% FIXME: explicit examples and comments about mapply, sapply, vapply
...
Recall that \texttt{apply} can take other arguments, as we see in the \activities\ after section \ref{mistery}.

For those of you who have programmed before: using the ``apply'' family is
often more efficient, and elegant, and easy to
understand, than using explicit loops.


\subsection{\activities}
\label{sec:activities-12}

Define a function that, when given a collection of numbers, will return
the ratio of the mean divided by the median. Using \texttt{apply} and
\texttt{for} (yes, we have already seen examples of \texttt{for}) compute
that function for the rows of Z and the columns of Z.
...
function on the fly):
<<tidy=FALSE>>=
(listA <- list(one.vector = 11:30,  hello = "Hola",
...
\subsubsection{\skipinclass{: lapply, more examples}}
Now, look at this, which is very similar, but here we access the second element:

<<>>=
(listB <- list(one.vector = 11:22, hello = c("Hola", "adios"),
               one.matrix = matrix(111:125, ncol = 5),
               another.list =
                 list(a = 5:9,
                      b = factor(c("male", "female", "female")))))

lapply(listB, function(x) x[2])

## Notice this too
lapply(listB, function(x) class(x))
@




\subsection{tapply}\label{sec:tapply-by}

When we have data (a column or several columns) that can be used to stratify or select other data (generally a vector), we often use \code{tapply}.
...
And we can form groups according to two or more variables
<<tapp1b>>=
...
\subsubsection{\textcolor {Green} {(Optional) More tapply and \texttt{by}}}

We can also use a function that returns more than one summary statistic:
...
But this, though it works, is probably not what we want:
...
\texttt{aggregate} (below) is probably what we'll want to use.


\texttt{by} will also work; \texttt{by} is related to \texttt{tapply}, but works on complete data frames and the look of the output is different:
...
by(one.dataframe,
   ...
   function(x) c(Mean_Age = mean(x$age), SD_Age = sd(x$age),
                 Median_Y = median(x$Y)))
@
% $ try to get latex shortcuts to behave with emacs

(\texttt{by} uses \texttt{split} underneath; we will see \texttt{split} in sections \ref{sec:split-} and in an example in \ref{sec:metab-rate-long}.)

% These next lines works, but it is too much detail

% ## by(one.dataframe[, c("age", "Y")],
% ##    list(one.dataframe$sex, one.dataframe$country),
% ##    function(x) c(Mean = mean(x), SD = sd(x)))
...
% by(cbind(age = one.dataframe$age,
%          Y = one.dataframe$Y),
%    list(one.dataframe$sex,
%               one.dataframe$country),
%    function(x) c(Mean = mean(x$age), SD = sd(x$Y)))
...
\code{aggregate} often returns things in a more convenient form:
...
## as we are using [], the second argument is already a list
aggregate(one.dataframe$age,
          one.dataframe[2], mean)
@

Can aggregate using two or more variables:
...
And we can use a function that returns several things:
...
% %% Why does the output give "x.Mean"? because that is the name of the argument. You could do

% <<>>=
% aggregate(data.frame(cucu = one.dataframe$age),
%           list(Sex = one.dataframe$sex,
%                Country = one.dataframe$country),
%           function(x) c(Mean = mean(x), SD = sd(x))
%           )
...
And you can use a formula interface which might be more intuitive. This is actually what I tend to use most of the time:
...
That will work also if what you want to compute functions of has multiple columns:
...
\subsubsection{\skipinclass{:\ \texttt{aggregate}, \texttt{with}, and being explicit about \texttt{FUN}}}
...
\activities: \ Repeat the above not being explicit about \texttt{FUN}
(i.e., passing the function but not explicitly passing it as explicit
argument to \texttt{FUN}). What
happens and why?

<<nofunaggregate,eval=FALSE>>=
with(one.dataframe,
     aggregate(age ~ sex,
               function(x) c(Mean = mean(x), SD = sd(x)))
     )
@
%% FUN is not passed, and it takes the function(x) as the data
%% argument


\subsubsection{\textcolor {Green} {(Optional) \texttt{aggregate}: supplementary details}}
\label{sec:texttt-suppl-deta}

(What follows are supplementary details.) Note, though, that the output from the last calls is not a dataframe of six columns: the output for age is a single dataframe column (that comprises a two-column matrix), and likewise for Y.
...
What can we do if we want a data frame with five columns? One option is to search (the internet) for functions that will do that. But the following is a much simpler alternative that uses base R with \texttt{do.call}:
...
Incidentally, this issue comes up from time to time in forums\footnote{\Burl{https://stackoverflow.com/a/17659731}, \Burl{https://stackoverflow.com/a/14759569}, \Burl{https://stackoverflow.com/a/12064297}, \Burl{https://stackoverflow.com/q/32561050}}.

% Another option involves using two calls and \texttt{cbind}ing the output:

% <<>>=

% call_1 <- aggregate(cbind(age, Y) ~ sex + country,
%                     data = one.dataframe,
%                     mean)

% call_2 <- aggregate(cbind(age, Y) ~ sex + country,
%                     data = one.dataframe,
%                     sd)
% ## Now, cbind the appropiate columns of call_1 and call_2
...
\subsubsection{\optactivities: \  aggregate, multiple columns, and xtabs. 0s and NAs}
\label{sec:activ-aggr-xtabs}

You can skip this; it will definitely not be in the exam. This can be useful, however, for more advanced uses where you combine \texttt{xtabs} and \texttt{aggregate} and you have 0s and/or NAs.

\begin{enumerate}
\item Create a data frame as follows:
<<agxtabs>>=
dfx2b <- data.frame(age = c(13, 13, 16, 25, 28),
                     sex = factor(c("male", "female",
                                    "female", "male", "male")))
dfx2b <- rbind(dfx2b, dfx2b)
dfx2b$Y <- runif(nrow(dfx2b))
dfx2b$Y[c(5, 10)] <- 0 ## we want to show a specific issue below

@

\item Obtain a data frame with the average of Y for all combinations of
  age and sex.
<<eval=TRUE,echo=FALSE,results='hide'>>=
aggregate(Y ~ age + sex, data = dfx2b, FUN = mean)
@

\item Using the data frame you just created, present the results as a
  table, using \texttt{xtabs} (maybe \texttt{xtabs} and
  \texttt{ftable}). It is even better (as you will reuse the code) if you
  do it without creating an explicit data frame. Please, try it before
  looking at the code!!!


  \vspace*{30pt}

<<xtabsagg6,eval=TRUE,echo=TRUE,results='show'>>=
xtabs(Y ~ age + sex,
      data = aggregate(Y ~ age + sex, data = dfx2b, FUN = mean))
@

What are those cells with 0? Do we really have 0 in all those cases?

\item Now, use argument \texttt{drop} to \texttt{aggregate} to make sure
  you do not get 0's where you should have NAs. Please, try it before
  looking at the code!!!

 \vspace*{30pt}
<<xtabsagg7,eval=TRUE,echo=TRUE,results='show'>>=

ftable(xtabs(Y ~ age + sex,
             data = aggregate(Y ~ age + sex, data = dfx2b,
                              FUN = mean, drop = FALSE)))

@

Where is the 0? Where are the NAs? Our data frame now contains NAs
for missing combinations. (Notice how the problems we saw in section \ref{xtabs-arbitrary} when using \texttt{xtabs} with missing combinations of some levels and data frame \texttt{dfx4} are now dealt with explicitly).


\item Turn the first value of Y into an NA. You now want the behavior of
  function \texttt{mean} to be such that if there is an NA in a set of values, you get the
  mean to be NA (not the mean of the values with the NA removed). Look at
  \texttt{na.action} in \texttt{aggregate}. Again, try before looking at
  the code.


<<xtabsagg8,eval=TRUE,echo=TRUE,results='show'>>=
dfx2b$Y[1] <- NA
...
Notice how that differs from (look at argument \texttt{na.action})
...
Note: some people are surprised by the use of \texttt{xtabs} on the
output from \texttt{aggregate}. But combining \texttt{xtabs} and
\texttt{aggregate} is an orthodox procedure: it is mentioned in the help page for \texttt{aggregate}.



\subsubsection{\textcolor {Green} {(Optional) More on aggregate and xtabs}}
(This is also supplementary material will not be in the exam, etc).

Some additional explorations of the combination of \texttt{xtabs} and \texttt{aggregate}. Using only one variable for aggregate or xtabs. What is the  difference? Compare carefully what is happening here

<<>>=
df7 <- expand.grid(location = c("A", "B"),
                   height = c("low", "med", "high"))
df7$radon <- runif(6, 1, 1000)
df7

xtabs(radon ~ height,
      data = aggregate(radon ~ height,
                       data = df7,
                       FUN = mean,
                       drop = FALSE))

xtabs(radon ~ height,
      data = aggregate(radon ~ height + location,
                       data = df7,
                       FUN = mean,
                       drop = FALSE))
@

(We did not use ftable in the previous example: it gives ugly output since
there is no table to flatten there)
...
overkill, but knowing about split can make life much simpler in many cases (for example, in  section \ref{sec:metab-rate-long})

<<>>=

lapply(split(one.dataframe, one.dataframe$sex),
       function(y) mean(y$age))



## Or (this is silly here, just to illustrate)

lapply(split(one.dataframe,
             list(one.dataframe$sex,
                  one.dataframe$country)),
       function(x) lm(Y ~ age, data = x))
...
\texttt{split} with \texttt{*apply} and some particular summary
function(s).  A long example of ``split-apply-combine'' is provided in
section \nameref{sec:metab-rate-long}.
...
(the original entry at \Burl{https://www.magesblog.com/post/2012-01-28-say-it-in-r-with-by-apply-and-friends/}).
For instance, the \code{doBy} package is really nice.

\subsubsection{\skipinclass{\ Further elaboration}}
<<>>=
## How is this different or similar from the call above?
## (hint: how are we telling lm where Y and age come from?)
lapply(split(one.dataframe,
             list(one.dataframe$sex,
                  one.dataframe$country)),
       function(x) lm(x$Y ~ x$age))

## And this is similar to some operations we did before
## with aggregate. Though I think aggregate's output
## and syntax is more convenient
lapply(split(one.dataframe, one.dataframe$sex),
       function(x) return(
                       c(mean_age = mean(x$age),
                         mean_y = mean(x$Y),
                         sd_age = sd(x$age))))
@
...
\subsection{apply and dropping dimensions from matrices}
...
\subsection{Other applies}
\label{sec:other-applies}

We will see \texttt{vapply} which is probably what you want to use in functions
and code instead of \texttt{sapply} (\texttt{sapply} is fine for interactive use).

\texttt{mapply} allows us to apply a function to multiple vector or list
arguments. We will use it in the examples discussed in
\texttt{Metabolic-rate-example.R}.

It is also possible to use apply over environments: \texttt{eapply}. We will not
use it in this course.

\subsection{Easily parallelized \texttt{apply} operations}
Look at package \texttt{parallel} (which is part of the basic packages) and
its function \texttt{mclapply}. (Under POSIX systems using forking is trivial; under Windows you'll need only a little more work).


\section{\textcolor {Green} {(Optional: three handy packages: \texttt{readr}, \texttt{dplyr},  \texttt{data.table})}}
...
We won't get into details about this: Hadley Wickham is the author of  two packages,
...
As for \texttt{readr}, check out \texttt{read\_table} and \texttt{read\_csv}; for very large data, I prefer \texttt{read\_table}.


Do not forget to look at package \texttt{data.table} for many big data manipulations. It is often my favorite for these tasks.
...
\R\  is a programming language. Comprehensive coverage is given in the books by Chambers ``Software for Data Analysis: Programming with R'',
...
but thorough introduction of the main concepts of programming in \R (as we said, this is a highly recommended document, which is part of every R installation).

It is important to emphasize that the ease of combining programming with
``canned'' statistical procedures gives \R\  a definite advantage over other languages in Bioinformatics (and explains its fast adoption).
...
solution: the \code{apply} family of functions ---see below--- is often a better approach.
...
---\verb@{}@--- after the \code{for} because the complete expression
...
\texttt{if}, \texttt{if ... else}, and \texttt{switch} are also available, of course. Regarding \texttt{if}, \texttt{if ... else} and \texttt{for}: I find it worth to \textbf{always} (almost always) use braces (\verb@{}@), even for statements in a single line: it minimizes mistakes and hard-to-debugg errors and makes it easier to understand the code.
...
\subsubsection{\skipinclass{: Confusing fun with loops }}

You can do confusing things. It is better to avoid them, and to avoid manipulation and using the loop index (though this is all explained in the help):

So the loop variable changes its value and takes the last value in the loop (or so it seems):
...
This, I would argue, is asking for trouble. I would always avoid this:
<<>>=
...
This is OK, though (but I would not name that variable as \texttt{i}, because things like \texttt{i,j,k} are often used as loop variables):
...
\subsubsection{\skipinclass{: Extra function examples, mixing plotting and ouput to the screen }}

Another example, that mixes plotting and a statistical function
<<>>=
...
and understand what is going on. Oh, what would change if the last statement were \verb@summary(x)@ instead of \verb@print(summary(x))@?).
...
arguments are considered. Really: type the above code. Those are
\activities !


\subsection{Scoping, frames, environments, etc}\label{scoping}
Understanding scoping is essential. Do this when you are awake. For now, we
will only scratch the surface. But yes, the material of this section can be part of the exam.
...
By the way, what do you think is more legible, the above function or one
that explicitly has \texttt{return(x + z)}?  (Yes, that is an \activities
).
...
\subsubsection{\activities }
\begin{itemize}
\item What will be the value of ``z'' used by f1? Think this before typing
  anything!

\item Make sure there are no objects named \texttt{z} in the workspace (e.g., \texttt{rm(z)}) and
  do \texttt{f1(3)}

\item Now assign \texttt{-100} to ``z'' and do \texttt{f1(3)}.


\item Now, do this:
<<f11z1,eval=TRUE,results='hide'>>=
z <- -100
...
@
\item Now, type this but \textbf{think about what you expect before
    pressing return!}:
<<f11z1ret>>=
f11(4)
@

\item And now type this. What is changing?
...
% handy and fun, but it can lead to surprises.
Good explanations here: \Burl{https://cran.r-project.org/doc/manuals/R-intro.html\#Scope} (An Introduction to R),
\Burl{https://socserv.socsci.mcmaster.ca/jfox/Books/Companion-1E/appendix-scope.pdf}
% \Burl{http://cran.r-project.org/doc/contrib/Fox-Companion/appendix-scope.pdf}
and \Burl{https://adv-r.hadley.nz/functions.html\#lexical-scoping}.
...
of using global variables in ``f1''. Using global variables is often regarded as poor
style so avoid that. It makes for bug-prone code.

There are issues of lexical scoping in what f2 and f3 are doing
(and I would regard f2 as a perfectly OK function, given how f3 is
defined, even if there is a free variable in f2). And all these relate,
and need to be answered, in the context of these concepts/terms (I take
here the definitions from the extremely clear PDF by John Fox, linked
above, with a few modifications following Chamber's ``Software for data
analysis'', p.\ 119 and ff.\ and chapter 8 of Wickham's ``Advanced R''):
...
\Burl{https://adv-r.hadley.nz/environments.html}. But maybe before that do
this
...
rm(c)
@
Oh, do you know what ``c'' we just removed? (Yes that was an \activities
).




If we have time, at the end of the course,  in section \ref{sec:scoping-more}, we will cover some of these issues in more detail using file \texttt{some-env-scoping-examples.R}.

%% FIXME: here we should go over global environment, search(), and attach()/detach() and explain that we can attach(df1) and the associated problems.



\subsection{\activities : not using global variables}
\label{sec:activities-13}

\begin{enumerate}
\item Rewrite our former \texttt{f1} without using global variables.
  (Yes, that is the very first \texttt{f1} we defined in \ref{scoping}).
\item Try to run the previous activities: which ones are affected and how?
  Why?
\item What type of code do you think is better: clearer, simpler to
  understand, and less likely to lead to bugs or surprises? (Search for
  ``principle of least surprise'').
\item Do these considerations apply to \texttt{f2} (the one defined inside
  f3)? Why or why not.
\end{enumerate}
...
\subsubsection{\activities}
\label{sec:activities-14}

Make sure you understand why only \texttt{fa} does what you want:
<<eval =FALSE, results='hide'>>=
...
\subsubsection{\activities \ : local}
\label{sec:activities--local}
When could you think of using \texttt{local}?

\subsection{An interesting consequence of lazy evaluation}\label{lazy}
...
Are you surprised? Regardless of it working, I would not do those things
routinely just for the fun of it.


(I have to confess, though, that I am surprised by \texttt{codetools} not saying anything about it)
...
(This is an awkward place to mention codetools. But we will use it again later. Moral for now: use \texttt{codetools} generously with your own code; in addition to things like \texttt{lintr} it will help you catch errors and bugs.)
...
\subsection{\textcolor {Green}{(Optional) Functions that return NULL and invisible}}
\label{sec:null}

Note: unless we have plenty of  time, we will skip this in class.
...
\begin{enumerate}
\item What are the differences between the three functions?
\item When could you want to use \textbf{fa}?
\item What is the difference between \texttt{cat} and \texttt{message}?
  Why one or the other?
\end{enumerate}



\subsection{\textcolor {Green}{(Optional) \ Using return explicitly and invisible}}
\label{sec:invisible_ret}

Note: unless we have plenty of  time, we will skip this in class.

Type the following code carefully (based upon \Burl{https://stackoverflow.com/a/38905341})
...
Compare the three versions. Is \texttt{f1w} blatantly redundant in some
parts? Does it help you spot the possible problem with \texttt{f1x} and
\texttt{f1z}? Where is the help in spotting the issues coming from?

(By the way, the key here is not \texttt{invisible} in itself, but rather
how we use \texttt{return})
...
\subsection{Commenting code}\label{sec:code-comments-2}
Please, make sure to read again the short section about ``Commenting code'': \ref{sec:code-comments}.
...
\section{Metabolic rate: a longer example with split-apply-combine}
\label{sec:metab-rate-long}

If time permits, we will go over the code in
\texttt{Metabolic-rate-example.R} (or\\ \texttt{Metabolic-rate-example-v2.R}, depending on time), to illustrate the split-apply-combine
strategy, filtering, function definitions, etc.

\subsection{\activities \ split-apply and plots example}
\label{sec:activities--split}

Grab the file \texttt{Metabolic-rate-example.R} or \texttt{Metabolic-rate-example-v2.R} from the repository
(\Burl{https://github.com/rdiaz02/R-bioinfo-intro}); either clone the repo, if
you use or would like to use git, or open the file as ``Raw'' and then copy and
download it \textbf{Do not just right-click on the file and download it} ---try
it to see what I mean: you will download an HTML file.
...
\Burl{https://web.archive.org/web/20170706215053/http://www.stats.uwo.ca:80/faculty/murdoch/software/debuggingR/}
...
\Burl{http://www.biostat.jhsph.edu/\%7Erpeng/docs/R-debug-tools.pdf},
chapter 22 in ``Advanced R, 2nd ed''
(\Burl{https://adv-r.hadley.nz/debugging.html} --- and this is from
chapter 9 in ``Advanced R, 1st ed'':
...
\activities: Really, do type the code!! The activities consist on typing
the code and seeing what is happening.
...
\subsubsection{\activities \ browser and debug main commands}
\label{sec:browser-debug-main}

Make sure you understand the difference between typing each of these while
under \texttt{browser()} or \texttt{debug()}. Write a function with a loop
and that calls another function and use \texttt{c} and
\texttt{s}

\begin{itemize}
\item \texttt{n} (and Enter, at least under Emacs/ESS)
\item \texttt{c}
\item \texttt{s}
\item \texttt{Q}
\item \texttt{f} (I rarely use it; I use \texttt{c} most of the time. A good
  explanation of the differences between \texttt{f} and \texttt{c} in
  \Burl{https://stackoverflow.com/a/37126663})%% finishes execution of the current loop or function
\end{itemize}
...
\code{trace} (see below).

It is actually even simpler (at least for me) to just edit the code and do
whatever I need. This just means using the \texttt{edit} option with trace:

<<eval=FALSE>>=
trace("lm", edit = "emacs")
trace("lm", edit = TRUE)
@
(if doing the second from Emacs, and using emacsclient, you might need to start
the server)
...
% stop the shitty font locking of the $$


You can, though, place code, such as \texttt{browser} at specific lines.
And how do you find the line number? Use
...
And, of course, and probably simpler

<<eval=FALSE, results = 'hide'>>=
trace(OncoSimulR:::nr_oncoSimul.internal, edit = TRUE)
@
...
four or five different systems for OOP. But  nothing in these notes (except for a possibly cryptic
reference to \code{predict.randomForest}, in section \ref{selbias})
requires you to understand any of this.

Being aware of OOP  can become
important when you write code and it will become relevant when you read
code from others. For details, look at section 10.9 in ``An introduction to R'' (\Burl{http://cran.r-project.org/doc/manuals/r-release/R-intro.html\#Object-orientation}), included with your R, which only covers S3, and the part of Wickham's 2nd edition of
``Advanced R'', \Burl{https://adv-r.hadley.nz/oo.html} (chapters 12 to 16), which
...
We will go over file \texttt{S3-S4-notes-simplified.R} as an
example. (That file is available from the repository).
...
simple; using them correctly is often subtle (e.g., are you really testing what you think you are testing? is the test sensitive to several different issues and could you end up rejecting the null for reasons that do not matter to you?), and there
are many scenarios/questions that might be hard to fit in them; \textit{caveat
  emptor}.
...
\item Generate data distributions according to the null hypothesis (\(H_0\)).
...
The above list allows us to construct a \textbf{``wish list''}: in the code file,
a (possibly commented) set of stubs we will fill up, as well as a list of
tests. When the wish list is empty, and all the tests are passed, we know we are
done. (As you move through the list, some items might get redefined).


<<>>=
## - Compute our statistic (here it is the difference in means).
##
## - Generate data distributions according to the null hypothesis.
##
## - Compute the statistic for each new data configuration.
##
## - Compare what we compute from our original data with what we obtain
##   under the null ($H_0$).
##
@
...
For steps 2 and 3 we will examine two ways of doing it. Make sure you understand
the differences
<<>>=
## numeric vector, numeric vector -> double (difference of means)
## Given two vectors, create two new vectors as random combination
##  of the set of observations into two groups, and return
##  the mean difference between those two groups.
##  [FIXME This description could be improved!!]
permut_and_mean1 <- function(x1, x2) {
    tmp <- sample(c(x1, x2))
    g1 <- tmp[seq(x1)]
...
<<>>=
set.seed(1)
permut_and_mean1(d11, d12)
permut_and_mean1(d11, d12)

set.seed(1)
permut_and_mean2(d11, d12)
permut_and_mean2(d11, d12)
@
...
We will probably \skipinclass{understanding the difference between the two ``permut\_and\_mean''}
...
set.seed(1)
null <- permut_and_mean1(d11, d12)
runif(1)

set.seed(1)
null <- permut_and_mean2(d11, d12)
runif(1)
...
set.seed(33)
null <- sample(length(d11) + length(d12))
runif(1)
...
set.seed(33)
null <- sample(1:(length(d11) + length(d12)), length(d11))
runif(1)

@


% (Now, we might want to take a look at \texttt{permut-test-2.R})
% %% Now, jump to permut-test-2.R



%% Clarify this. Or not. Steps of runif and steps of sample are not the same.
% <<>>=
% ## This first
% set.seed(13457)
% runif(40)
...
% null <- sample(10)
% runif(10)
% test_runif <- function(n, after = 5, seed = sample(1:1e4, 1)) {
%   set.seed(seed)
%   null <- sample(n)
%   post <- runif(after)
%   set.seed(seed)
...
% runif(1)
% set.seed(1)
% null2 <- runif(length(d11) + length(d12) )
% runif(1)
% set.seed(1)
% null <- permut_and_mean2(d11, d12)
% runif(1)
% set.seed(1)
% null2 <- runif(length(d11))
% runif(1)
% set.seed(1)
% null <- sample(1:10)
% runif(1)
...
After we are done understanding the differences between the two \texttt{permut\_and\_mean}, let us come back to writing a single  function:
<<>>=
## FIXME!! A Function signature is missing!!!

permut_testA <- function(data1, data2,
                         fsample = permut_and_mean1,
                         num.permut = 100) {
    obs.stat <- mean_d(data1, data2)
...
passing a function as an argument\footnote{As for the denominator and
  ...
  implementations get it wrong and do not include it). Why to do this is explained in, for example,
Manly, B. F. J. (2007). Randomization, bootstrap and monte carlo methods in biology, 3rd ed. Chapman \& Hall; Noreen, E. W. (1989). Computer-intensive methods for testing hypotheses : An introduction. Wiley; Edgington, E., \& Onghena, P. (2007). Randomization tests (4th ed.). Chapman and Hall/CRC; Phipson, B., \& Smyth, G. K. (2010). Permutation p-values should never be zero: Calculating exact p-values when permutations are randomly drawn. Stat Appl Genet Mol Biol, 9, Article39. }


Is it working?
<<>>=
tmp <- permut_testA(d11, d12)
tmp <- permut_testA(d21, d22)
@
It looks like it is.
...
<<>>=
tmp <- permut_testA(d11, d11)
tmp <- permut_testA(d12, d12)
@
...
permut_testB <- function(data1, data2,
                         fsample = permut_and_mean1,
                         num.permut = 100) {
    obs.stat <- mean_d(data1, data2)
...
tmp <- permut_testB(d11, d11)

tmp <- permut_testB(d12, d12)
@
...
stopifnot(suppressMessages(permut_testB(d11, d11))$pv == 1)
stopifnot(suppressMessages(permut_testB(d12, d12))$pv == 1)
xr <- runif(10)
stopifnot(suppressMessages(permut_testB(xr, xr))$pv == 1)
...
test_that("p-value = 1 when the two vectors identical", {
    ...
    for (i in 1:ntests) {
        xr <- rnorm(10)
        ## Or should we use expect_equal?
        expect_identical(permut_testB(xr, xr)$pv, 1)
    }
})
...
And we might want to make sure that the size of \texttt{xr} is itself variable.

And we would probably want to use a larger value for \texttt{num.permut}.
...
<<>>=
## FIXME: function signature
permut_test <- function(data1, data2,
                        fsample = permut_and_mean1,
                        num.permut = 100) {
  obs.stat <- mean_d(data1, data2)
  permut.stat <- replicate(num.permut,
...
par(mfrow = c(2, 1))
tmp <- permut_test(d11, d12, num.permut = 1000)
tmp <- permut_test(d21, d22, num.permut = 1000,
                   fsample = permut_and_mean2)
@


You might want to compare with t-tests, etc.

You might also want to improve the message: way too many decimals.


And, of course, \textbf{we should add some more tests}. These are a minimal set
of tests (testing functions with a stochastic element can be complicated). And we
test the final function. (These tests generate lots of figures, which is slightly
annoying; we might want to do something there).
...
test_that("p-value = 1 when the two vectors identical", {
    ...
    for(i in 1:ntests) {
        xr <- rnorm(10)
        expect_identical(permut_test(xr, xr)$pv, 1)
    }
})
...
test_that("p-value is 1 when same elements, different length", {
    ...
    for(i in 1:n) {
        ...
        rx1 <- rep(e, 5)
        rx2 <- rep(e, 15)
        expect_true(permut_test(rx1, rx2, num.permut = 1000)$pv == 1)
    }
})
...
test_that("p-value is < 1 when extreme differences", {
    ...
    for(i in 1:n) {
        rxx <- runif(numcases)
        rxy <- rxx + 1000
        expect_true(permut_test(rxx, rxy, num.permut = 1000)$pv < 1)
    }
})

@

More complex testing (comparing p-value distribution against a uniform
distribution when null is true, comparing against specific alternatives, etc)
would be desirable.
...
FIXME: to be written in more detail.

We will examine a couple of packages in class, explaining the structure of the
tar.gz and zip files and what gets placed in your local hard drive, for both
packages with and without code written in other languages. And we will look at
one remote repository of a package.

\begin{itemize}
\item Does the package need compilation: CRAN info
\item Does the package use external libraries: CRAN info, BioC info
\item Directories and files in a package that does not need compilation.
  \begin{itemize}
  \item tar gz
  \item zip file
  \end{itemize}
\item Directories and files in a package that needs compilation.
  \begin{itemize}
  \item tar gz
  \item zip file
  \end{itemize}
\item Directories and files in our local hard drive: what gets installed
\item Vignettes, PDF of the documentation.
\item An example repo

\end{itemize}


\section{A few things to practice}
These are some miscellaneous things that we have mentioned, but maybe not
emphatically enough. \textbf{Make sure you try the following!!!}:


\begin{itemize}
\item Difference between \texttt{scan} and \texttt{read.table}: see the help
\item Using \texttt{mapply}. For example
  \begin{verbatim}
   mapply(rep, 11:14, 4:1)
  \end{verbatim}
\item \texttt{args}
\item \texttt{sapply} and \texttt{vapply}. From \texttt{Metabolic-rate-example.R}
<<eval=FALSE>>=
i39 <- sapply(3:9, seq)
i39b <- i39
...
## And the very first sapply? Hard to do with vapply
mapply(seq, from = 1, to = 3:9)
Map(seq, from = 1, to = 3:9)
@

\end{itemize}

\section{Merging: merge, join}
\label{sec:merging:-merge-join}

(In development. We will cover this if time permits. For now, I just show
the code. This is from file \texttt{merge-join.R})
...
\subsection{do.call, rbind, rbindlist}
\label{sec:do.c-rbind-rbindl}

Here we comment on why we might want to do things like \texttt{do.call(rbind, something)}; then, why, with large data, we might want instead \texttt{data.table::rbindlist} or \texttt{dplyr::bind\_rows}.

We saw a call to \texttt{do.call(rbind, something)} in section \ref{sec:texttt-suppl-deta}. \red{To be written, as need arises}. In the mean time, you can check these posts (I do not necessarily endorse them, and some of the use the pipe which, as is widely known, I dislike): \Burl{https://stackoverflow.com/questions/10801750/whats-the-difference-between-lapply-and-do-call}, \Burl{https://rstudio-pubs-static.s3.amazonaws.com/886639_d5edefc1d5b341a4ab3aa2b2c37289cb.html}, \Burl{https://rstudio-pubs-static.s3.amazonaws.com/406521_7fc7b6c1dc374e9b8860e15a699d8bb0.html}.

%% Locate the examples

\subsection{Reshaping data: moving between long and wide formats}
\label{sec:resh-data:-moving}
See \texttt{reshape-long-wide.R}


\subsection{More scoping and lexical scope}
\label{sec:scoping-more}
See \texttt{some-env-scoping-examples.R}
