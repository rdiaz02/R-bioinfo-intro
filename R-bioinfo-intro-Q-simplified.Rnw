...
Try to run the above with a \code{\&} instead of \code{\&\&}.
...
Play with the following and make sure you understand (at least
the first part)
...
get a quick idea of the spread of the points, etc. I want to do it only for the birds.
...
Finally, do a regression of log-transformed variables and plot data and line, but just for the birds:
...
Note, though, that the output from the last calls is not a dataframe of five columns: the output for Sex is a single dataframe column (that comprises a two-column matrix), and likewise for Country.
...
\subsection{Easy parallelized \texttt{apply} operations}
...
under Windows you'll need something different).
...
But do not forget to look at package \texttt{data.table} for many big data manipulations. It is often my favorite.
\clearpage
