...
Some sections are marked as ``\activities''. Please make sure to do them. Some exam questions will come from the \activities.

Some others are marked as ``\optactivities'': we rarely have time to cover or discuss them; you are encouraged to try them, but unless explicitly stated, we won't cover this in class (nor, if there is an exam, will they be part of an exam). Other sections/subsections/whatever that we will not cover in class (nor would be in an exam) are indicated in  \textcolor{Green}{green}.


\subsubsection{Changes starting on 2025-10-16: moving material to \skipinclass{whatever}}
\label{sec:chang-start-2025}

As we are (course 2025-2026) going too slow, I try to concentrate only the \textbf{absolutely, nonnegotiable, essential stuff} and move lots to subsections (or subsubsubsections, or whatever). The material moved out is indicated as ``\skipinclass{Whatever (subsub)section title}''. \textbf{I would NOT skip this while studying}; I am skipping it in class (unless you have questions), for the sake of efficiency, but this is not optional material; it is just material that introduces some redundancy. You can expect that some exam questions will come from the \skipinclass{skipped in class material}. (Again: the only optional material (the material that will not be in the exam), is indicated as \optactivities\  or has section (subsection, or whatever) in \textcolor{Green}{green}.)
...
\subsection{\textcolor {Green} {Tables II}}
\label{tablesII}
...
\subsubsection{\textcolor {Green}  {Recovering a table from a data frame}}
<<>>=
...
\subsubsection{\textcolor {Green} {Recovering the data that produced the table}}
...
\subsection{\textcolor {Green} {xtabs and ftable: summarizing arbitrary stuff}}\label{xtabs-arbitrary}
...
\subsection{\textcolor {Green} {Tables and missing values, more examples and details}}
\label{tablena}
...
\subsection{\textcolor {Green}{Tables, III}}\label{tablesIII}
Unless we have a lot of time:
\begin{itemize}
\item We will skip this in class.
\item This will not be in the exam.
...
\subsubsection{\textcolor {Green} {More tapply and \texttt{by}}}
...
\subsubsection{\textcolor {Green} {\texttt{aggregate}: supplementary details}}
...
\subsubsection{\textcolor {Green} {More on aggregate and xtabs}}
...
by Chambers ``Software for Data Analysis: Programming with R'',
...
using the loop index (though this is all explained in the help):
...
This is OK, though (but I would not name that variable as \texttt{i})
...
This is more advanced material. Do this when you are awake. For now, we
...
If we have time, we will cover some of these issues in more detail in file \texttt{some-env-scoping-examples.R}.
...
(I have to confess, though, that I am surprised by \texttt{codetools} not saying
anything about it)
...
(This is an awkward place to mention codetools. But we will use it again later)
...
\subsection{\optactivities \ Functions that return NULL and invisible}
\label{sec:null}
...
\subsection{\optactivities \ Using return explicitly and invisible}
