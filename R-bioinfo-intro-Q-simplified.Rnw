...
Some sections are marked as ``\activities''. Please make sure to do them.
...
As we are (course 2025-2026) going too slow, I try to concentrate only the \textbf{absolutely, nonnegotiable, essential stuff} and move lots to subsections (or subsubsubsections, or whatever). The material moved out is indicated as ``\skipinclass{Whatever (subsub)section title}''. \textbf{I would NOT skip this while studying}; I am skipping it in class (unless you have questions), for the sake of efficiency, but this is not optional material. (It is just material that introduces some redundancy.)  The optional material (the material that will not be in the exam), is indicated as \optactivities\  or has section (subsection, or whatever) in \textcolor{Green}{green}.
...
\activities \  Can you think why? Try to come up with some examples or google around. Really.
This is a short section: Please, please, please, never, ever, use \code{T} for \code{TRUE} or \code{F} for \texttt{FALSE}. You can count on at least one exam question about this.
...
## TRUE and FALSE cannot be overwritten
TRUE <- 0
FALSE <- 1
...
\subsubsection{Shortcircuit logical operations}
...
From R-4.4.1 this is now an error: ``Using vectors of more than one element in '\&\&' or '||' will give an error.''
...
Notice the warning. And notice that the returned value has length 1. Wait, was that a warning or was that an error?
...
## What are these doing? Avoid these things
ages[c(FALSE, TRUE)]
...
So, recommendation: do not try to be too smart when dealing with lists. Access each type of element, and then subset that element as needed.
...
\subsection{\activities: matrices indexing matrices}

Play with the following and make sure you understand it (at least
the first part)
...
The basic plot function is \code{plot}. It's \textbf{help} can be slightly
misleading and many additional arguments are explained in \code{par}.
...
This shows a few common elements. Again, this is just a very tiny set of what is available..

<<plotssimple>>=
plot(x, y, xlab = "This is the label for the x axis",
     ylab = "Label for the y axis",
     main = "A title", pch = c(19, 1:9),
     cex = 1:10,
     col = rainbow(10))
text(2, 3, "Some text", srt = 45)
abline(a = 3, b = 2.5, lty = 2, col = "blue")
abline(a = 13, b = -5, lty = 4, col = "salmon", lwd = 4)
...
\subsubsection{\skipinclass{:\ More details}}
...
\skipinclass{We will skip all of this for now. I will only briefly explain random number generators and using the seed.}
...
<<eval=TRUE,echo=FALSE,results=FALSE>>=
...
\activities \ And what happens if you do
...
\subsection{tapply (and by)}\label{sec:tapply-by}
...
(What follows are supplementary details:)

Note, though, that the output from the last calls is not a dataframe of six columns: the output for age is a single dataframe column (that comprises a two-column matrix), and likewise for Y.
