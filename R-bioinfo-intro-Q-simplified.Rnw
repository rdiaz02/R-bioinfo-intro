...
Some sections are marked as ``\activities''. Please make sure to do them. Some others as ``\optactivities'': we rarely have time to cover or discuss them; you are encouraged to try them, but unless explicitly stated, we won't cover this in class (nor, if there is an exam, will they be part of an exam).


\subsubsection{Changes starting on 2025-10-16: moving material to \skipinclass{whatever}}
\label{sec:chang-start-2025}

As we are (2025-2026) going too slow, I try to concentrate only the \textbf{absolutely, nonnegotiable, essential stuff} and move lots to subsections (or subsubsubsections, or whatever). The material moved out indicated as ``\skipinclass{Whatever (subsub)section title}''. \textbf{Do NOT skip this while studying}; I am skipping it in class (unless you have questions), for the sake of efficiency, but this is not optional material.
(This will, at least for now, introduce some redundancy.)
...
its function \texttt{mclapply}. (Under POSIX systems using forking is trivial;
under Windows you'll need only a little bit more work).
