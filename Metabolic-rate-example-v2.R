...
## We want to produce plots of the relationship between
## metabolic rate and body mass, adding regression lines,
## when our data contain birds and reptiles.
...
### Filter
## Either will do. Below (search for "Filter, checking") we examine
## in more detail and go over an apparent error when removing NAs
birds <- anage[anage$Class == "Aves", ]

library(dplyr)
birds2 <- dplyr::filter(anage, Class == "Aves")

### split-apply, vapply, stopifnot: ablines and regression

## Let us learn to do these plots manually
## In this case it is not necessary, but what if you were fitting
## a new, bespoke model?
...
## What if, by accident, we have different kinds of data?
i39b <- i39
i39b[[2]] <- factor(letters[1:5])

## sapply happily (and incorrectly?) does it
sapply(i39b, summary)
...
vapply(i39b, summary,
       rep(0, 6))
## Eh! Pay attention to the message!!
## FUN(X[[2]]) result is length 5
## It is telling us X[[2]]
...
## We illustrate making sure to operations yield the same result
## and use this example to illustrate mapply, Map, stopifnot,
## and looking at attributes
...
## Ramon: Remember this won't work in Emacs
## if plotting using xwidgets or essgd or EAF
legend(locator(1), legend = levels(anage$Class),
       col = c("salmon", "darkgreen"),
       pch = c(1, 2))


### Using scatterplot and trying to replicate it using plot
...
## (For details of stringsAsFactors: https://developer.r-project.org/Blog/public/2020/02/16/stringsasfactors/index.html)
