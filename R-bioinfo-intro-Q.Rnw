...
(\Burl{http://socserv.mcmaster.ca/jfox/Misc/Rcmdr/}) which is a basic statistics
GUI for R. In this course we will rarely (if at all) use R Commander, since these
notes are focused on programming and using R from the command line. However, I do
recommend that you play around with R
