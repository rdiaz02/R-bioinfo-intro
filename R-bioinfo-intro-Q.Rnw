...
And, of course, we should add some more tests. These are a minimal set of tests
(testing functions with a stochastic element can be complicated). And we test the
final function. (These tests generate lots of figures, which is slightly
