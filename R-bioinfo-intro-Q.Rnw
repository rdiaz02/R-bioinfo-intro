...
\item All you have now is bare bones Emacs. What about starter kits? In other words, ``Emacs bundles of packages + config with all batteries included''? I do not generally recommend them. Some people (mainly ex-Vim users) enjoy Emacs Doom or Spacemacs, but they might later get in the way.
  If I had to recommend one, I'd try ``Emacs Bedrock'': \url{https://sr.ht/~ashton314/emacs-bedrock/}; it uses some settings and packages I do not use, but seems very minimal, and it looks like it would  easily allow modifying the configuration ``as the user grows in knowledge and power''.
...
\item To learn more about Emacs itself:
  \begin{itemize}
    ...
    \item The short intro course ``Getting started with Emacs'', by
      Marie-Hélène Burle, available from
      \url{https://mint.westdri.ca/emacs/top_intro}.

    \item The book ``Use GNU Emacs. The Plain Text Computing Environment'',
      by Keith Waclena:
      \url{https://www2.lib.uchicago.edu/keith/emacs/}. Freely available as
      HTML, PDF, Epub ---yeah, there are unfinished chapters, but it is
      very complete as it is now.
...
\item I'd start with the short intro course to get a quick overview and
  then dive into one of Petersen's or Waclena's, or both. And I'd probably intersperse jumping to the Emacs manual to read specific chapters or sections of things that particularly interest you (e.g., read about Dired in Petersen or Waclena, then go to the Emacs Manual). Repeat as needed.
  \end{itemize}
