...
and that calls another function and use \texttt{c} and \texttt{f} and
\texttt{s}
...
\item \texttt{s}
\item \texttt{Q}
\item \texttt{f} (I don't use it; I use \texttt{c})%% finishes execution of the current loop or function
\end{itemize}
