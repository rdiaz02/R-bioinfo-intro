...
\texttt{if}, \texttt{if ... else}, and \texttt{switch} are also available, of course. Regarding \texttt{if}, \texttt{if ... else} and \texttt{for}: I find it worth to \textbf{always} (almost always) use braces (``{}''), even for statements in a single line: minimizes mistakes and hard-to-debugg errors and makes it easier to understand the code.
