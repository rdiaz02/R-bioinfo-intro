...
\item[Calling R from the shell] Open a shell, a command window, or however
  ...
  instead of \code{ > script1.Rout}). In Windows, you might need to use \verb@
  Rscript.exe @ or \verb@ R.exe @.
...
Please, please, please, never, ever, use \code{T} for \code{TRUE} or \code{F} for
\texttt{FALSE}.
