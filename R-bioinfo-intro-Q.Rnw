...
Suppose we have a data frame some arbitrary variable for (some of) the
levels of some categorical variable. But we want a table (pay attention:
...
I would argue that the last table, with \texttt{dfx4}, is probably not
...
That will work also if what you want to compute functions of has multiple
columns:
...
\subsubsection{\activities: \  aggregate, multiple columns, and xtabs}
...
Where is the 0? Where are the NAs?


Notice how the problems we saw in section \ref{xtabs-arbitrary} when using
\texttt{xtabs} with missing combinations of some levels and data frame
\texttt{dfx4} are now dealt with explicitly: our data frame contains NAs
for missing combinations.


\item Turn the first value of Y into an NA. You now want the behavior of
  mean to be such that if there is an NA in a set of values, you get the
...
\item Using only one variable for aggregate or xtabs. What is the
  difference? Compare carefully what is happening here
...
\texttt{aggregate} is an orthodox procedure: it is mentioned in the help
page for \texttt{aggregate}.)
...
overkill, but knowing about split can make life much simpler in many cases:
