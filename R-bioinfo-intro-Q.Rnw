...
\item Copy the file ``AnotherDataSet'' and create a bunch of different
  ...
  name you want.
\item You want to use the following types of changes:
  \begin{enumerate}
  \item Replace one or more values by ``NA''
  \item Replace one or more values by a `` '' (a white space)
  \item Replace several values by nothing.
    \begin{enumerate}
    \item Make sure you have at least two ``nothings'' one after the other.
    \end{enumerate}
  \end{enumerate}
