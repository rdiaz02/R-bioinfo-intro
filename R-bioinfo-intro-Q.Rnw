...
other special character string to identify them. \textbf{You will most
  ...
  explicit), specially for character variables.}
...
\item Read the documentation for \texttt{readRDS} (and \texttt{saveRDS}). Then:
  ...
  \end{enumerate}
\item When could \texttt{save/readRDS} be more useful than \texttt{save/load}? % assign names of objects at will
...
\item[Calling R from the shell] Open a shell, a command window, or however
  ...
  This article contains details and examples: \Burl{https://swcarpentry.github.io/r-novice-inflammation/05-cmdline/}.
...
\activities \  Can you think why? Try to come up with some examples or google
around. Really. We will show a few examples in class.

This is a short section: Please, please, please, never, ever, use \code{T} for
\code{TRUE} or \code{F} for \texttt{FALSE}.
...
If you go back to the argument \texttt{arr.ind} this should make sense:
...
\subsection{Recovering a table from a data frame}
<<>>=
...
\subsection{Recovering the data that produced the table}
...
\subsection{xtabs and ftable: summarizing arbitrary stuff}\label{xtabs-arbitrary}


(This is here for completeness, but we are unlikely to cover this in class. Specially the examples with NAs are tricky.)
...
\subsubsection{\texttt{aggregate}: supplementary details}
...
\subsubsection{\activities: \  aggregate, multiple columns, and xtabs. 0s and NAs}
...
\subsubsection{More on aggregate and xtabs}
...
\section{(Optional: three handy packages \texttt{readr} and \texttt{dplyr} and \texttt{data.table})}
