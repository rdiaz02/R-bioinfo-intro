...
\item After that, we jump into \R\ with three longer examples (section
  ...
  much of what is done, but we will go over it in class.
...
\item[Programming] As it says, just focus on programming R:
  ...
  \item \textit{Advanced R}. Wickham. (If you go to the web page for the
    ...
    own pdf).
  \end{itemize}
\item[Stats and some programming] Introductory statistics with some
  programming interleaved.
  \begin{itemize}
  \item \textit{Introductory statistics with R, 2nd ed}. Dalgaard.
  \item \textit{R in Action}. Kabacoff. (A second ed.\ should soon be
    available).
  \end{itemize}
...
\clearpage
\subsection{Those common genes and some Venn diagrams}
