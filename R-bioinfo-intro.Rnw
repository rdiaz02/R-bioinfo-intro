...
the ``reshape2'' package (and possibly also the ``tidyr'' package) tends
...
compressed file \texttt{permafrost.zip}).
...
sequences <- lapply(files, function(x) scan(x, what = ""))
@ 
...
characters and then tabulate; we just want the frequency of T.

<<>>=
fnucl <- function(x) {
    indivc <- strsplit(x, "")[[1]]
    table(indivc)[4]/nchar(x)
}
@ 
...
\subsection{How did I generate the data?}
