...
\section{Programming: debugging}


\section{Programming: lexical scope}
...
passing a function as an argument\footnote{As for the denominator and
  ...
  detail, and it is true that it makes little difference as $n \rigtharrow
...
Well, it is \textbf{wrong}, but we might not have noticed it. What p-value
would you expect?
<<>>=
tmp <- permut.test1(d11, d11)
tmp <- permut.test1(d12, d12)
@ 
Moral: check, check. One should check against different scenarios. (In
fact, the above might often look correct; try running the comparison of
d11 with itself with 100000 permutations).
