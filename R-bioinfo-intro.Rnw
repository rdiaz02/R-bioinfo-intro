...
\title{A quick introduction to R with a bioinformatics bent}
...
the previous commands with the up-arrow (\thinspace $\uparrow$ \thinspace)
...
\clearpage
\section{Plotting the results from BLAST}\label{blast}
...
\clearpage
\section{A simple hypothesis test: the \textit{t-}test}
...
\subsection{The t-tests and some plots}
<<>>=
GeneA <- rnorm(50)
round(GeneA, 3)
...
\section{Metabolic rate and body mass: a regression example}
...
\subsection{A plot with changed scale}
...
\subsection{Only the birds!}

Eh, but I want only the birds. Let's select only the birds using function \code{subset}
...
\subsection{The regression only for the birds}
...
\subsection{How I read and save the data?}\label{readanage}
This is all I did
<<eval=FALSE>>=
...
Plotting is a big thing in \R. We have used a few, but there are many
more. In fact, there are several approaches or systems for plots in \R. We
will use here the basic one (the one in base \R), but notice that
...
you can use commands such as \code{sink}.



\subsection{Saving an \R session: .RData}
...
\section{Scripts and non-interactive runs}

\subsection{Why scripts}

\subsection{Paths: where are scripts and data sets located}
Before you can tell \R\  to use your script or read some data, you need to
tell \R\  where, exactly, to find the scripts/data.  Here are a couple of
ways to do it:
...
You will be using either one of the two approaches above. It is up to
you. I will be using the second: I will be running \R\  in the very same
directory where the data files and scripts are located. (You can assume
that I have, sometime in the past, issued a \code{setwd} command.)
...
And what are the Arithmetic functions? Type \code{?Arithmetic},
\code{?log}, \code{?exp}, \code{?Trig}
...
were numeric: we can treat \code{TRUE} as 1 and
\code{FALSE} as 0. (Note that we can also treat anything larger than 0 as TRUE).


This can be very handy to find out how many elements
fulfill a condition.
...
\section{Interlude: comparing floats}
...
\section{Matrices}
\subsection{Creating matrices from a vector}

\subsection{Combining vector to create a matrix: \code{cbind, rbind}}
\label{sec:comb-vect-create}

\subsection{Matrix indexing and subsetting}
...
\subsection{Dropping dimensions}
