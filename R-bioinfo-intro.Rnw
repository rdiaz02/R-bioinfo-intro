...
Finally, this very simple and elegant solution was suggested in class by
zz: 
...
trueIfMatch1 <- function(lx) {
    allgenes <- sort(unique(unlist(lx)))
    return(sapply(lx, function(z) {allgenes  %in% z } ))
}
@ 

or, better yet, using vapply:

<<>>=


trueIfMatch2 <- function(lx) {
    allgenes <- sort(unique(unlist(lx)))
    vapply(lx, function(z) {allgenes  %in% z } ,
           logical(length(allgenes)))
}
@ 
...
geneFiles2Mat2 <- function(prefix = "Condition_", 
                          postfix = "txt") {
    l1 <- readListGenes(prefix = prefix, postfix = postfix)
    return(trueIfMatch2(l1))
}
