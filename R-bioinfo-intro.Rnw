...
\date{\gitAuthorDate\ {\footnotesize (\gitVtag; Rev: \gitAbbrevHash)}}
...
\item We then jump into \R\ with three longer examples (section
  \ref{crashex}). Again, on first reading you will not understand much of
...
\item But you do not want to skip section \ref{debug}. This section covers
  debugging and being able to debug quickly and painlessly is essential
  for using R effectively (and enjoy it).
...
Note that for many commands I do not show the output (e.g., because it
would just provide boring and space-filling output). However, make sure
you type and understand it. You can copy of past, of course, but I
...
\item \code{R-bioinfo-intro-html-dir.zip}: a zip file that, when
  ...
  of this PDF and a directory for the figures (used in the HTML).
...
Reshaping data: Sometimes you will need to go from a ``wide''
