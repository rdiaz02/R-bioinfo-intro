...
\item I then show a six-line real example of a common problem in
  Bioinformatics (multiple testing). You will not understand much of what
  is done, but I will explain it in class (this is not a textbook, but a
  document for a class).
\item We then go over a few practical things we need to get out of the way
  (sections \ref{basics} and \ref{console}).
\item We then jump into \R\ with three longer examples (section
  \ref{crash}). Again, on first reading you will not understand much of
  what is done, but we will go over it in class.
...
\item The rest are more advanced or specific features and also include
  some more examples.
\end{itemize}
...
frustration when working through sections \ref{mistery} and \ref{crash},
but definitely look at them before class and try to understand what is
going on. After section \ref{crash} things should be smoother (but more
boring). And here, definitely, you \textbf{must} type things on your own
and understand the output. I have tried to use a kind of spiraling lay
out, working over several things repeatedly, and repeating and going
deeper on some key ideas. Hopefully, this will allow you to understand the
material better, connect it with other pieces, and retain it for longer.




\subsection{The PDF and the HTML and the code, etc}

The primary output of this document is a PDF. I also provide (and will
use) and HTML file; this is kind of experimental (a few things might not
...
less fiddling around that using a PDF). In the HTML, the code is in red
...
github repo you will not see the PDF, HTML, or R-bioinfo-intro.R files,
...
\item[Programming] As it says, just focus on programming R:
  ...
  \item \textit{Advanced R}. Wickham. (If you go to the web page for the
    ...
    own pdf).
  \end{itemize}
\item[Some programming, some stats] Some introductory statistics, some
  programming, both more or less interleaved.
  \begin{itemize}
  \item \textit{Introductory statistics with R, 2nd ed}. Dalgaard.
  \item \textit{R in Action}. Kabacoff. (A second ed.\ should soon be available)
  \end{itemize}
\item[Linear models et al.] Linear models are fundamental in
  ...
  \item \textit{Regression modeling strategies}. Harell. (But I've heard
    for sometime that eventually a 2nd ed.\ will be available). Among its
    many virtues, this book contains 
...
Emacs + ESS (\Burl{ess.r-project.org/}). For those used to Eclipse, there
...
\Burl{http://www.sciviews.org/\_rgui/}, and some other entries around in
internet land are
...
Many functions include executable code (examples)
<<eval=FALSE>>=
example(rnorm)
example(graphics)
example(lm)
...
\clearpage
\section{Three practical, crash examples}\label{crashex}
...
legend(5, 5, legend = levels(anage$Class), col = c("salmon", "darkgreen"),
       pch = 1)
@ 
...
\paragraph Reshaping data 
Sometimes you will need to go from a ``wide''
to a ``long'' format in your data, or viceversa (e.g., repeated measures
of the same individuals over time). Look at \code{?reshape}, although I
find that the ``reshape2'' package tends to make life much easier for this
task.

\paragraph Data table
For dealing efficiently with large data sets, and other niceties, the
``datatable'' package (with its datatables) can really make a difference.
...
<<lty2,fig.cap='Those line types (lty) again'>>=
...
definitions from the extremely clear PDF by John Fox, linked above):
...
\item[environment] A sequence of frames. When f2 goes looking for the
  value of ``v'' it will do so looking through a sequence of frames. R
  will search through a sequence of frames, so understanding how that
  sequence is ordered is crucial to understanding the above examples.
\end{description}


And this would bring us to consider, too, why things such as this work
at all in the way we expect:
...
before we leave, we will remove ``c'':
<<>>=
rm(c)
@ 
...
\section{The ``apply'' family, ``aggregate'', etc}
...
\section{Debugging and catching exceptions}
...
what follows, I provide just a very quick summary of really helpful things
that I use often.  But there is much more. Look here: 
...
\code{traceback} shows the call stack and helps you identify where the
sequence of calls lead to a problem:
...
you wrote, wherever you want it to stop. 
...
Make sure you try this, and when it stops, look around (e.g., type
\code{ls()}, reassign values, etc).
...
including, say, from a package you have not written? Yes. You just have to
pass it the line number, using the function \code{trace}. And how do you
find the line number? Use \code{as.list(body(theFunctionname)}.
...
when you only get errors from time to time, etc\footnote{The chapter from
  ``Advanced R'' would seem to suggest you do \code{options(error =
    browser)}. Nope, you have to do \code{options(error = recover)}.}.
...
in this area. In addition, this is like to just get more and more
functionality in the future (comfortable and powerful debugging are
important features).
...
ft <- function(x) {
    ...
    if(inherits(tmp, "try-error")) {
        message(paste("It looks like you did something wrong.",
                tmp))
    } else{
        return(tmp)
    }
...
\section{Additional programming practice}
...
\clearpage
\subsection{Some Venn diagrams of genes}
Someone has examined what genes are over-expressed in three different
conditions (three different drugs). They are called ``Condition\_A.txt'',
``Condition\_B.txt'', ``Condition\_C.txt''. You want to understand which
are over-expressed under more than one drug. You might want to know which
are overexpressed in three conditions, or which are overexpressed in
exactly these two conditions, etc. Or find out how many are overexpressed
under two of the drugs but not the third. Etc.
...
names? Let's assume only ``Condition\_'' is fixed. And this thing of calling
\code{intersect} many times \ldots ugly too. And it is hard to see how
many are common to A and B, and A and C, etc easily. So we would like a
matrix that has, as columns, the conditions, and as rows the gene
names. In each entry, a TRUE means overexpressed, and a FALSE not-overexpressed.
...
<<>>=
## This is the union of all gene names and contains a single entry of each
## name. We do not use "union" but "unique", which is simpler to use here.
...
not. If we\footnote{I find it better to be as explicit as possible. An
  alternative would be to return a 1 of type integer, using ``1L'', and a
  0 of type integer (0L).}. In
other words, we want a vector for each experiment with the positions
...
<<>>=
overexpressed <- vapply(ovxpGenes, function(x) f1(x, all.the.genes), 
...
a common prefix and end in a common postfix. We will use default values, though.
...
Now, place everything in a matrix. First, give \code{f1} a better name
...
be warranted. See section \ref{test}. We've done some testing about, but
...
(are you really testing what you think you are testing?), and there are
many scenarios/questions that might be hard to fit in them (i.e., they are
not \textbf{the} solution to the world's problems); \textit{caveat emptor}.
...
(\Burl{http://en.wikipedia.org/wiki/Test-driven_development}). Maybe you
do not need/want to follow it completely, but writing and using tests
should be a must for any serious project. In R there are a couple of
...
Run it
<<fig.cap='Two examples of permutation tests'>>=
par(mfrow = c(1, 2))
...
\clearpage
\subsection{Selection bias in classification}
...
\item The above are default arguments. But enlightenment will come easily
  (i.e., you should) play around with those: is selection bias more severe
  the more genes you select? The more genes you can select from? What
  about number of subject? Etc, etc.
