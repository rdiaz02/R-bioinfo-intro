...
STOP! We gave three possible ways of doing this, with \texttt{f1},
\texttt{f2}, \texttt{f3}: compare the output of the three (maybe using
\texttt{identical} or, for programming, inside functions,
\texttt{stopifnot}).
