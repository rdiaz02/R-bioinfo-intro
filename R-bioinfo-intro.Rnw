...
\item We then (sections \ref{readingr} to \ref{plotsplots}) cover in some
  ...
  how to manipulate them, a few quick ideas about programming, and some
  plotting. This can be boring, and this used to be after section
  \ref{crashex} but some students argued strongly that they'd rather see
  this first, so this comes now first.
...
\item[Programming] As it says, just focus on programming R:
  ...
  \item \textit{R Programming for Data Science}. Peng. (This is an
    ebook and PDF, and you can pay whatever you want for it.)
...
R is often used to produce plots as it can produce a variety of plots and
they can be modified at will. In this course we will only scratch the
surface. You will have a change to get additional practice from the exercises.
...
%%% FIXME: move immediately after tables
...
%% FIXME: use the formula for aggregate


%% FIXME: use with
=======
%% FIXME: examples with the formula interface to aggregate
...
\texttt{lapply} there might be preferable.
